    ...
    e.widget("ui.selectable", e.ui.mouse, {options:{appendTo:"body", autoRefresh:true, distance:0, filter:"*", tolerance:"touch"}, _create:function () {
        ...
        this.refresh = function () {
            ...
            f.each(function () {
                ...
                e.data(this, "selectable-item", {element:this, $element:c, left:b.left, top:b.top, right:b.left + c.outerWidth(), bottom:b.top + c.outerHeight(), startselected:false, selected:c.hasClass("ui-selected"), selecting:c.hasClass("ui-selecting"), unselecting:c.hasClass("ui-unselecting")})
            })
        };
        this.refresh();
        ...
        this.dragged = true;
        if (!this.options.disabled) {
            var c = this.options, b = this.opos[0], g = this.opos[1], h = d.pageX, i = d.pageY;
            if (b > h) {
                var j = h;
                h = b;
                b = j
    ...
    d.widget("ui.sortable", d.ui.mouse, {widgetEventPrefix:"sort", options:{appendTo:"parent", axis:false, connectWith:false, containment:false, cursor:"auto", cursorAt:false, dropOnEmpty:true, forcePlaceholderSize:false, forceHelperSize:false, grid:false, handle:false, helper:"original", items:"> *", opacity:false, placeholder:false, revert:false, scroll:true, scrollSensitivity:20, scrollSpeed:20, scope:"default", tolerance:"intersect", zIndex:1E3}, _create:function () {
        ...
        if (this.options.scroll) {
            ...
            if (this.scrollParent[0] != document && this.scrollParent[0].tagName != "HTML") {
                ...
                if (a.pageY - d(document).scrollTop() < b.scrollSensitivity)c = d(document).scrollTop(d(document).scrollTop() - b.scrollSpeed); else if (d(window).height() - (a.pageY - d(document).scrollTop()) < b.scrollSensitivity)c = d(document).scrollTop(d(document).scrollTop() + b.scrollSpeed);
...
(function (c) {
    ...
    c.extend(c.ui.accordion, {version:"1.8.1", animations:{slide:function (a, b) {
        ...
        } else a.toHide.animate({height:"hide"}, a); else a.toShow.animate({height:"show"}, a)
...
(function (a) {
    ...
    a.widget("ui.button", {options:{text:true, label:null, icons:{primary:null, secondary:null}}, _create:function () {
        ...
        if (this.type === "input")this.options.label && this.element.val(this.options.label); else {
            var b = this.buttonElement, c = a("<span></span>").addClass("ui-button-text").html(this.options.label).appendTo(b.empty()).text(), d = this.options.icons, e = d.primary && d.secondary;
...
(function (d) {
    ...
    d.widget("ui.tabs", {options:{add:null, ajaxOptions:null, cache:false, cookie:null, collapsible:false, disable:null, disabled:[], enable:null, event:"click", fx:null, idPrefix:"ui-tabs-", load:null, panelTemplate:"<div></div>", remove:null, select:null, show:null, spinner:"<em>Loading&#8230;</em>", tabTemplate:'<li><a href="#{href}"><span>#{label}</span></a></li>'}, _create:function () {
        ...
        if (c == "selected")this.options.collapsible && e == this.options.selected || this.select(e); else {
            this.options[c] = e;
            this._tabify()
        }
    }, _tabId:function (c) {
...
(function ($) {
    $.fn.swipe = function (options) {
        ...
        var defaults = {fingers:1, threshold:75, swipe:null, swipeLeft:null, swipeRight:null, swipeUp:null, swipeDown:null, swipeStatus:null, click:null, triggerOnTouchEnd:true, allowPageScroll:"auto"};
        var LEFT = "left";
        var RIGHT = "right";
        var UP = "up";
        var DOWN = "down";
        ...
        var hasTouch = 'ontouchstart'in window, START_EV = hasTouch ? 'touchstart' : 'mousedown', MOVE_EV = hasTouch ? 'touchmove' : 'mousemove', END_EV = hasTouch ? 'touchend' : 'mouseup', CANCEL_EV = 'touchcancel';
        var phase = "start";
...
(function ($) {
    $.fn.autofill = function (text_value) {
        return this.each(function () {
            var obj = $(this);
            if (text_value == null) {
                text_value = obj.attr("title");
            }
            if (document.activeElement != obj[0]) {
                obj.val(text_value);
            }
            obj.focus(function () {
                if (obj.val() == text_value) {
                    obj.val('');
                }
            }).blur(function () {
                if (obj.val() == '') {
...
(function ($) {
    $.fn.carousel = function (params) {
        ...
        return this.each(function () {
            var env = {$elts:{}, params:params, launchOnLoad:[]};
            env.$elts.carousel = $(this).addClass("js");
            env.$elts.content = $(this).children().css({position:"absolute", "top":0});
            env.$elts.wrap = env.$elts.content.wrap('<div class="carousel-wrap"></div>').parent().css({overflow:"hidden", position:"relative"});
            env.steps = {first:0, count:env.$elts.content.children().length};
            env.steps.last = env.steps.count - 1;
            if (env.params.buttonPosition = "below") {
                env.$elts.buttonrow = $(this).parent().children(".carousel-button-row");
                if (env.$elts.buttonrow.length < 1) {
                    env.$elts.buttonrow = $(this.appendChild(document.createElement("div"))).addClass("carousel-button-row");
                    $(this).append(env.$elts.buttonrow);
                }
                if (env.params.buttonsAlign == 'right') {
                    env.$elts.buttonrow.addClass("LayoutHAlignRight");
                    env.$elts.buttons = $(env.$elts.buttonrow[0].appendChild(document.createElement("div"))).addClass("LayoutHAlignItems");
                    env.$elts.buttonrow.append(env.$elts.buttons);
                }
                ...
                env.$elts.buttons.addClass("carousel-buttons");
                ...
                if (env.params.buttonsAlign == 'right') {
                    env.$elts.prevBtn.wrap('<div class="LayoutHItem"></div>');
                    env.$elts.nextBtn.wrap('<div class="LayoutHItem"></div>');
                }
                else if (env.params.buttonsAlign == 'split') {
                    env.$elts.prevBtn.wrap('<div class="LayoutHItem"></div>');
                    env.$elts.nextBtn.wrap('<div class="LayoutHItemRight"></div>');
                }
            }
            ...
            $(function () {
                ...
                env.itemWidth = $firstItem.outerWidth(true);
                ...
                env.$elts.content.width(env.contentWidth + 1);
                ...
                if (params.direction == "vertical") {
                    ...
                    env.$elts.content.parent().css({height:env.itemHeight + "px", display:"block"});
                    env.$elts.carousel.css({height:env.itemHeight + "px"});
                }
    ...
    function initPagination(env) {
        if (env.params.btnsPosition = "below") {
            env.$elts.pagination = $('<div class="center-wrap LayoutHItem carousel-pagination"></div>')[((env.params.paginationPosition == "outside") ? "insertAfter" : "prependTo")](env.$elts.buttons);
        }
        env.$elts.paginationBtns = $([]);
        if (env.params.paginationType == 'single') {
            var pageTotal;
            env.$elts.content.find("li").each(function (i) {
                if (i % env.params.dispItems == 0) {
                    env.$elts.paginationBtns = env.$elts.paginationBtns.add($('<span class="UIHide"><a role="button">' + env.params.tabLabel(env.$elts.paginationBtns.length + 1) + '</a></span>').data("firstStep", i));
                    pageTotal = env.$elts.paginationBtns.length;
                }
            });
            env.$elts.paginationBtns.length = 0;
            env.$elts.content.find("li").each(function (i) {
                if (i % env.params.dispItems == 0) {
        ...
        env.$elts.paginationBtns.css({display:"none"});
        env.$elts.paginationBtns.each(function () {
            $(this).appendTo(env.$elts.pagination);
        });
        env.$elts.paginationBtns.slice(0, 1).addClass("active Link LinkPointer");
    ...
    function updateButtonsState(env) {
        ...
        if (env.params.pagination && env.params.paginationType == "all") {
            ...
            }).addClass("active").removeClass("Link LinkPointer");
        }
        if (env.params.pagination && env.params.paginationType == "single") {
            env.$elts.paginationBtns.removeClass("active").addClass("Link LinkPointer carousel-hidden").filter(function () {
                return($(this).data("firstStep") == env.steps.first)
            }).addClass("active").removeClass("Link LinkPointer carousel-hidden");
        }
    }
...
(function ($) {
    ...
    var $jc = $.jcarousel;
    $jc.fn = $jc.prototype = {jcarousel:'0.2.7'};
    ...
    $jc.fn.extend({setup:function () {
        ...
        e.addClass(this.className('jcarousel-item')).addClass(this.className('jcarousel-item-' + i)).css({'float':(this.options.rtl ? 'right' : 'left'), 'list-style':'none'}).attr('jcarouselindex', i);
        return e;
    }, className:function (c) {
        return c + ' ' + c + (!this.options.vertical ? '-horizontal' : '-vertical');
    }, dimension:function (e, d) {
        var el = e.jquery !== undefined ? e[0] : e;
        var old = !this.options.vertical ? (el.offsetWidth || $jc.intval(this.options.itemFallbackDimension)) + $jc.margin(el, 'marginLeft') + $jc.margin(el, 'marginRight') : (el.offsetHeight || $jc.intval(this.options.itemFallbackDimension)) + $jc.margin(el, 'marginTop') + $jc.margin(el, 'marginBottom');
        if (d == null || old == d) {
            return old;
        }
...
$(document).ready(function () {
    $(".CustomSelect").jqDropDown();
});
(function ($, window) {
    var DropDown = function (element, settings) {
        var $dropdown = $(element), settings = $.extend({}, $.fn.jqDropDown.defaults, settings), listWidth, $container, defaultOption = settings.defaultOption, selectedLabel, $toggleLink, $options, effect = settings.effect, effectSpeed = settings.effectSpeed, $optList = $('<ul class="' + settings.optionListName + '" style="display:none;"></ul>');
        ...
        $toggleLink.bind('mousedown keydown focusout click', function (e) {
            var $this = $(this), $targetOption, selectedIndex = $this.data('index') || 0, indexChanged = false, $selectedOption, $options = $optList.find('a'), key;
                ...
                switch (e.keyCode) {
                    ...
                    case 13:
                        $(document).trigger('mousedown', [
                            {target:$optList.find('li:eq(' + $this.data('index') + ') a:eq(0)')}
                        ]);
                        break;
                    case 38:
                        e.preventDefault();
    ...
    function BuildOptionList($list, settings) {
        var optList, i, listItem = '', $curItem, isSelected = '';
    ...
    function ApplyStyle($container, lisWidth, settings) {
        var $list = $container.find('ul:eq(0)'), $listItems = $container.find('ul > li > a'), $toggleLink = $container.find('a:eq(0)'), toggleLinkPadding = parseInt($toggleLink.css("padding-left"), 10) + parseInt($toggleLink.css("padding-right"), 10), defaultOpt_padding;
        UpdateListDirection($toggleLink, $list, settings);
        if (settings.defaultStyle) {
            $container.find('li.' + settings.optionGroupName).length > 0 ? defaultOpt_padding = 25 : defaultOpt_padding = 10;
            $toggleLink.css({width:lisWidth - toggleLinkPadding}).append('<span class="CPodSelectLinkArrow"></span>');
            $list.css({width:200 + toggleLinkPadding});
            $listItems.css({'padding-left':defaultOpt_padding});
        }
    }
    ...
    function UpdateListDirection($toggleLink, $list, settings) {
        ...
        if (direction === 'up' || (($toggleLink.offset().top + $list.height() + 20) > $(window).height() + $(window).scrollTop()) && ($toggleLink.offset().top > $list.height() + 20)) {
    ...
    $.fn.jqDropDown.defaults = {effect:'default', effectSpeed:400, modal:false, data:{}, defaultOption:null, containerName:'ddContainer', toggleBtnName:'ddToggle', optionListName:'ddOptionList', optionGroupName:'optgroup', optionName:'ddOption', defaultStyle:true, placeholder:null, useValue:true, direction:'down', beforeToggle:function () {
    ...
    $.format = (function () {
        ...
        var parseTime = function (value) {
            ...
            if (values3.length === 3) {
                ...
                minute = values3[1];
                second = values3[2];
                return{time:retValue, hour:hour, minute:minute, second:second};
            } else {
                return{time:"", hour:"", minute:"", second:""};
            }
        };
        return{date:function (value, format) {
            try {
                var year = null;
                var month = null;
                var dayOfMonth = null;
    ...
    $.extend($.fn, {selectBox:function (method, data) {
        ...
        var init = function (select, data) {
            ...
            control.addClass(select.attr('class')).attr('style', select.attr('style') || '').attr('title', select.attr('title') || '').attr('tabindex', parseInt(select.attr('tabindex'))).css('display', 'inline-block').bind('focus.selectBox',function () {
                ...
                control.find('.selectBox-arrow').addClass('LayoutFlipV');
            }).bind('blur.selectBox', function () {
                if (!control.hasClass('selectBox-active'))return;
                control.removeClass('selectBox-active');
                select.trigger('blur');
                control.find('.selectBox-arrow').removeClass('LayoutFlipV');
            });
            ...
            if (inline) {
                ...
                label.attr('title', ($(select).find('OPTION:selected').attr('title') || ' '));
                ...
                var menuDropWidth = select.next('.selectBox-dropdown').width();
                var menuPanelWidth = select.width();
                var menuWidth;
                if (menuDropWidth > menuPanelWidth) {
                    menuWidth = menuDropWidth;
                }
                else {
                    menuWidth = menuPanelWidth;
                }
                control.css('width', (menuWidth - 13) + "px");
                var optionsWidth = control.width() + 12;
        ...
        var getOptions = function (select, type) {
            ...
            switch (type) {
                case'inline':
                    options = $('<ul class="selectBox-options TextSmall" />');
                    ...
                    return options;
                case'dropdown':
                    options = $('<ul class="selectBox-dropdown-menu selectBox-options TextSmall" />');
                    if (select.find('OPTGROUP').length) {
                        select.find('OPTGROUP').each(function () {
                            var optgroup = $('<li class="selectBox-optgroup" />');
                            optgroup.text($(this).attr('label'));
        ...
        var showMenu = function (select) {
            ...
            options.css({top:control.offset().top + control.outerHeight() - borderBottomWidth, left:control.offset().left});
        ...
        var setValue = function (select, value) {
            ...
            control.find('.selectBox-label').attr('title', ($(select).find('OPTION:selected').attr('title')));
...
$(document).ready(function () {
    $('body').append('<div class="ModalCPodSelect" />');
    var modalCPod = $(".ModalCPodSelect");
    modalCPod.dialog({modal:false, bgiframe:true, autoOpen:false, resizable:false, draggable:false, closeOnClick:false, closeOnEscape:true, width:200, dialogClass:'CPodSelectDialog', height:'auto', close:function () {
        modalCPod.empty();
        $('body').removeClass('ModalCPodSelectPanel');
        modalCPod.dialog('option', 'position', ['center', 'center']);
    }});
    $('.ModalCPodSelect-button').click(function (event) {
        modalCPod.dialog("close");
        $('body').addClass('ModalCPodSelectPanel');
        var offsetX = (event.clientX - 100);
        modalCPod.dialog('option', 'position', [offsetX, event.clientY]).load(this.href, function () {
        ...
        modalCPod.focus();
        modalCPod.dialog('open');
        return false;
    });
    $('.ModalTip-button-inline').click(function (event) {
        $('body').addClass('ModalCPodSelectPanel');
        var urlAry = this.href.split("#");
        var hiddenDiv = urlAry[1];
        $('.ModalCPodSelect').empty().html($('#' + hiddenDiv).html());
        $('.ModalCPodSelect').dialog('option', 'position', [event.clientX, event.clientY]);
        $('.ui-dialog-close').hover(function () {
            $(this).addClass("ui-state-hover");
            return false;
        }, function () {
    ...
    $('.ModalCPodSelect .CPodSelectAccordian .CPodContent li a').live("click", function (event) {
        event.preventDefault();
        modalCPod.dialog("close");
    });
});
$(document).ready(function () {
    $('body').append('<div class="ModalTip" />');
    var modalTip = $(".ModalTip");
    modalTip.dialog({modal:false, bgiframe:true, autoOpen:false, resizable:false, draggable:false, closeOnClick:false, width:340, height:'auto', close:function () {
        modalTip.empty();
        $('body').removeClass('ModalTipPanel');
        modalTip.dialog('option', 'position', ['center', 'center']);
    }});
    $('.ModalTip-button').click(function (event) {
        $('body').addClass('ModalTipPanel');
        modalTip.dialog('option', 'position', [event.clientX, event.clientY]).load(this.href, function () {
            $('.ui-dialog-close').hover(function () {
                $(this).addClass("ui-state-hover");
                return false;
            }, function () {
        ...
        modalTip.focus();
        modalTip.dialog('open');
        return false;
    });
    $('.ModalTip-Hover').bind({mouseenter:function () {
        var ipHeight = $('#CheckoutProdInfo').height();
        $('#CheckoutProdInfoShadow').height(ipHeight + 14);
        $('#CheckoutProdInfoShadow').show();
        $('#CheckoutProdInfo').show();
        if ($('#SiteFooter') != null) {
            $('#RegionPanel').hide();
        }
    }, mouseleave:function () {
        $('#CheckoutProdInfoShadow').hide();
        $('#CheckoutProdInfo').hide();
    }});
    $('.ModalTip-button-inline').click(function (event) {
        $('body').addClass('ModalTipPanel');
        var urlAry = this.href.split("#");
        var hiddenDiv = urlAry[1];
        modalTip.empty().html($('#' + hiddenDiv).html());
        modalTip.dialog('option', 'position', [event.clientX, event.clientY]);
        $('.ui-dialog-close').hover(function () {
            $(this).addClass("ui-state-hover");
            return false;
        }, function () {
...
(function ($, c, b) {
    $.map("click dblclick mousemove mousedown mouseup mouseover mouseout touchstart touchend touchmove change select submit keydown keypress keyup".split(" "), function (d) {
        a(d)
    });
    a("focusin", "focus" + b);
    a("focusout", "blur" + b);
...
(function ($) {
    $.extend({__stringPrototype:function (str) {
        ...
        this.inspect = function (useDoubleQuotes, s) {
            s = this.s(s);
            var specialChar = this.specialChar, escapedString = this.gsub(/[\x00-\x1f\\]/,function (match) {
...
(function () {
    $.widget("jv.treeList", {options:{selectable:true}, _create:function () {
        var w = this, control = $(this.element);
        w._initItem(control.find('dl'));
        var $lisOpen = control.find('dt.ui-treeList-open');
        w.openNode($lisOpen);
        control.find('a.treeList').bind('click keypress', function (event) {
            if (event.type == 'keypress' && event.which != '13') {
                return;
            }
            var treeButtonLength = control.find('dt').length, treeButtonsClosed = control.find('dt.ui-state-default').length;
            if (treeButtonsClosed < treeButtonLength) {
                event.preventDefault();
                w.closeNode(control.find('dt'));
            }
            else {
                event.preventDefault();
                w.openNode(control.find('dt'));
            }
        });
        control.find('dt a.ui-treeList-toggle').bind('click keypress', function (event) {
            if (event.type == 'keypress' && event.which != '13') {
                return;
            }
            var t = $(event.target).parents('dt');
            if (t.hasClass('ui-state-default')) {
                event.preventDefault();
                w.openNode(t);
            }
            else {
        ...
        $(this.element + 'a.treeList').unbind('click');
        ...
        $lis.addClass('ui-treeList ui-widget ui-widget-content').find('dt').addClass('ui-treeList-item ui-widget-content ui-state-default').hover(function () {
            ...
            $(this).removeClass('ui-state-hover');
            return false;
        }).wrapInner('<div class="TreeListFloat" />').prepend('<div class="TreeListArrow"><span class="ui-widget ui-widget-content ui-icon ui-icon-triangle-1-e"></span></div>').wrapInner('<div class="LayoutBreakAfter TreeListPad" />').end().children('dd').addClass('TreeListPad').hide().end().find('dt a').addClass('ui-treeList-toggle');
    }, openNode:function ($lisOpen) {
        if ($lisOpen) {
            $lisOpen.removeClass('ui-state-default').next('dd').show().end().find('.ui-icon-triangle-1-e').removeClass('ui-icon-triangle-1-e').addClass('ui-icon-triangle-1-s');
            $lisOpen.find(".TreeListPreview").hide();
        }
    }, closeNode:function ($lisClose) {
        if ($lisClose) {
            $lisClose.addClass('ui-state-default').next('dd').hide().end().find('.ui-icon-triangle-1-s').removeClass('ui-icon-triangle-1-s').addClass('ui-icon-triangle-1-e');
            $lisClose.find(".TreeListPreview").show();
        }
    }});
})(jQuery);
$(document).ready(function () {
    $('.SignInLink').live('click', function () {
        $('.CreateNewLink').removeClass("ActiveLink");
        if (!($(this).hasClass("ActiveLink"))) {
            $(this).addClass("ActiveLink");
            $(".CreateNewContent").hide();
            $(".SignInContent").show();
        }
    });
    $('.CreateNewLink').live('click', function () {
        $('.SignInLink').removeClass("ActiveLink");
        if (!($(this).hasClass("ActiveLink"))) {
            $(this).addClass("ActiveLink");
            $(".SignInContent").hide();
            $(".CreateNewContent").show();
        }
    });
});
$(document).ready(function () {
    $(".SubscriptionCheckout").click(function () {
        if ($(this).hasClass("DisableFade")) {
            return false;
        }
    });
    $(".TermsConditions").click(function () {
        if ($(this).is(":checked")) {
            ...
            $(".SubscriptionCheckout").addClass("DisableFade");
        }
    });
    $("#CartPromotionApply").click(function () {
        if ($(this).hasClass("DisableFadeGrey")) {
            return false;
        }
        else {
        }
    });
    $("#CartPromotion").keyup(function () {
        if (!($(this).val() == "")) {
            $("#CartPromotionApply").removeClass("DisableFadeGrey");
        }
        else {
            $("#CartPromotionApply").addClass("DisableFadeGrey");
        }
    });
    $("#CartShippingApply").click(function () {
        if ($(this).hasClass("DisableFadeGrey")) {
            return false;
        }
        else {
    ...
    $("#ShippingCode").keyup(function () {
        if (!($(this).val() == "")) {
            ...
            $("#CartShippingApply").addClass("DisableFadeGrey");
        }
    });
});
URLParser = (function () {
    ...
    var isWorkarea = (pathArray[1] == "WORKAREA");
...
$(document).ready(function () {
    ...
    function bitLy() {
        if ($.browser.msie) {
            ...
            var account = $('meta[name=area]').attr("content");
            switch (account) {
                case'devnet':
                    var bitLyLogin = 'edsulliva';
                    var bitLyApiKey = 'R_3466dc7c929862c7b400574c56d3eb6b';
                    break;
                case'products':
                    bitLyLogin = 'adobesm';
                    bitLyApiKey = 'R_214c6d683f4ce59a161ce18046883b5d';
                    break;
                default:
                    bitLyLogin = 'adobesm';
                    bitLyApiKey = 'R_214c6d683f4ce59a161ce18046883b5d';
            }
            var bitLyApi = 'http://api.bit.ly/v3/shorten?';
            var bitLyFormat = 'xml';
            var bitLyURL = bitLyApi + 'login=' + bitLyLogin + '&apiKey=' + bitLyApiKey + '&longUrl=' + encodeURIComponent(PageToolsUrl) + '&format=' + bitLyFormat;
            $.ajax({type:'GET', contentType:'application/x-www-form-urlencoded', dataType:bitLyFormat, url:bitLyURL, async:false, success:function (xml) {
                $(xml).find("url").each(function () {
                    bitLySuccess = true;
                    bitLyShortURL = $(this).text();
                });
            }, error:function () {
            }});
        }
    ...
    $('.Facebook-button').click(function (event) {
        ...
        if (bitLySuccess) {
            ...
            window.open('http://www.facebook.com/sharer.php?u=' + encodeURIComponent(PageToolsUrl) + '&t=' + encodeURIComponent(PageToolsTitle), 'sharer', 'toolbar=0,status=0,width=626,height=436');
        }
    });
    $('.Twitter-button').click(function (event) {
        ...
        if (bitLySuccess) {
            ...
            window.open('http://twitter.com/share?text=' + encodeURIComponent(PageToolsTitle) + '&url=' + encodeURIComponent(PageToolsUrl), 'sharer', 'toolbar=0,status=0,width=775,height=436');
        }
    });
    $('.LinkedIn-button').click(function (event) {
        ...
        if (bitLySuccess) {
            ...
            window.open('http://www.linkedin.com/shareArticle?mini=true&url=' + encodeURIComponent(PageToolsUrl) + '&title=' + encodeURIComponent(PageToolsTitle), 'sharer', 'toolbar=0,status=0,width=626,height=436');
        }
    });
    $('.PayPal-button').click(function (event) {
        event.preventDefault();
        window.open('https://www.paypal.com/us/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsPayPal-outside', 'olcwhatispaypal', 'toolbar=0,status=0,width=400,height=350');
    });
    if ($.browser.msie || $.browser.mozilla || window.opera) {
        $('.Bookmark-button').click(function (event) {
            event.preventDefault();
            var BookmarkUrl = parent.location.protocol + '//' + location.host + window.location.pathname;
    ...
    $('.RouterClose').click(function () {
        var IconTarget = $(this).attr('rel');
        $('#' + IconTarget).hide("pulsate", {times:1}, 600);
    });
    $('.LinkDownload').hover(function () {
        $(this).find('a').css('cursor', 'default');
        var DownloadHover = $(this).find('a').attr('rel');
        $(this).addClass("Link" + DownloadHover);
    }, function () {
        $(this).find('a').css('cursor', '');
        var DownloadHover = $(this).find('a').attr('rel');
        $(this).removeClass("Link" + DownloadHover);
    });
    $('.RouterMore').hover(function () {
        $(this).removeClass("RouterMore");
        $(this).addClass("RouterMoreHover");
    }, function () {
        $(this).removeClass("RouterMoreHover");
        $(this).addClass("RouterMore");
    });
    $('.RouterEducation').hover(function () {
        $(this).removeClass("RouterEducation");
        $(this).addClass("RouterEducationHover");
    }, function () {
        $(this).removeClass("RouterEducationHover");
        $(this).addClass("RouterEducation");
    });
    $('.RouterFacebook').hover(function () {
        $(this).removeClass("RouterFacebook");
        $(this).addClass("RouterFacebookHover");
    }, function () {
        $(this).removeClass("RouterFacebookHover");
        $(this).addClass("RouterFacebook");
    });
    $('.RouterTwitter').hover(function () {
        $(this).removeClass("RouterTwitter");
        $(this).addClass("RouterTwitterHover");
    }, function () {
        $(this).removeClass("RouterTwitterHover");
        $(this).addClass("RouterTwitter");
    });
    $('.RouterRSS').hover(function () {
        $(this).removeClass("RouterRSS");
        $(this).addClass("RouterRSSHover");
    }, function () {
        $(this).removeClass("RouterRSSHover");
        $(this).addClass("RouterRSS");
    });
    $('.RouterAIGA').hover(function () {
        $(this).removeClass("RouterAIGA");
        $(this).addClass("RouterAIGAHover");
    }, function () {
        $(this).removeClass("RouterAIGAHover");
        $(this).addClass("RouterAIGA");
    });
});
adobe = {version:"1.0", release:"dotcom", msgs:{TYPERR:"argument is not of type "}, vrbls:{}, vrbl:function (name, value) {
    if (name === undefined) {
        return adobe.vrbls;
    }
    if (value === undefined) {
        return adobe.vrbls[String(name)];
    }
    return adobe.vrbls[String(name)] = value;
}};
adobe.dom = {};
adobe.dom.createStyleSheet = function (htmlheadelement, href) {
    if (htmlheadelement === undefined || htmlheadelement.nodeType != 1) {
...
adobe.dom.getStyleSheetsByProperty = function (prop) {
    var sheets = [];
    for (var i = 0, l = window.document.styleSheets.length; i < l; i++) {
        if (window.document.styleSheets[i].hasOwnProperty(prop)) {
            sheets.push(window.document.styleSheets[i]);
        }
    }
    return sheets;
};
adobe.dom.disableStyleSheet = function (sheet) {
...
adobe.dom.enableStyleSheet = function (sheet) {
    if (sheet === undefined || sheet === null) {
        return sheet;
    }
...
adobe.dom.CSSStyleSheet = function (href) {
    this.setHref(href);
    this.disabled = false;
    this.element = null;
};
adobe.dom.CSSStyleSheet.prototype = {setHref:function (href) {
    this.href = href.toString();
}, enable:function (htmlheadelement) {
    if (this.element == null) {
        this.element = adobe.dom.createStyleSheet(htmlheadelement, this.href);
    } else {
        adobe.dom.enableStyleSheet(this.element);
    }
    this.disabled = false;
}, disable:function () {
    adobe.dom.disableStyleSheet(this.element);
    this.disabled = true;
}, toString:function () {
    return"[Object adobe.dom.StyleSheet]"
}};
adobe.fn = {};
adobe.fn.ApexPageFilter = function (e) {
    var menuBarID = "#" + e, menuBar = $(menuBarID), menuControlID = "MenuControl", menuControl = $('#' + menuControlID);
    if (menuControl[0] == null) {
        $('<div/>', {id:menuControlID}).appendTo('body');
        menuControl = $('#' + menuControlID);
    }
    var menuDropWidth = $(menuBarID + ' .MenuDrop').width();
    var menuPanelWidth = $(menuBarID + ' .MenuDropPanel').width();
    var menuWidth;
    if (menuDropWidth > menuPanelWidth) {
    ...
    $(menuBarID + ' .MenuDrop').css('width', '230px');
    $(menuBarID + ' .MenuDropPanel').css('width', '230px');
    var menuCheckHover = function (target) {
        if (menuControl.hasClass('MenuClose')) {
            target.find("div[class*='MenuDrop']").removeClass("MenuDropHover").unbind('mouseenter mouseleave');
            target.find("div[class='MenuDrop']").addClass("MenuDropOpen");
        }
        else {
            target.find("div[class*='MenuDrop']").removeClass("MenuDropOpen");
            target.find("div[class='MenuDrop']").hover(function () {
                $(this).addClass("MenuDropHover");
            }, function () {
                $(this).removeClass("MenuDropHover");
            });
        }
    };
    menuCheckHover(menuBar);
    menuBar.find("div[class*='MenuDropName']").bind('click', function () {
        var menuPanel = menuBar.find("div[class*='MenuDropPanel']");
        if (menuControl.hasClass('MenuClose')) {
            $('.MenuDropPanel').fadeOut('fast').removeClass('MenuShow');
            menuControl.removeClass('MenuClose');
        }
        if (menuPanel.hasClass('MenuShow')) {
            menuPanel.fadeOut('fast').removeClass('MenuShow');
            menuControl.removeClass('MenuClose');
            menuCheckHover(menuBar);
        }
        else {
            menuPanel.show(000).addClass('MenuShow');
            menuControl.addClass('MenuClose');
            menuCheckHover(menuBar);
            menuControl.unbind('click');
            menuControl.bind('click', function () {
                $(menuBarID + ' .MenuDropPanel').fadeOut('fast').removeClass('MenuShow');
                menuControl.removeClass('MenuClose');
                menuCheckHover(menuBar);
            });
            $('body').unbind('keyup');
    ...
    var menuDropItem = new Array();
    $(menuBarID + ' .MenuDropItem').each(function (i) {
        $(this).attr('id', e + 'MenuDropItem' + [i]);
        menuDropItem[i] = '#' + $(this).attr('id');
        if ($(menuBarID + ' .MenuDropItemSelected').size() == 0) {
            $(menuDropItem[0]).addClass('MenuDropItemSelected');
        }
        $(menuDropItem[i]).bind('click', function () {
            $(menuBarID + ' .MenuDropItem').removeClass('MenuDropItemSelected');
            $(this).addClass('MenuDropItemSelected');
        });
    });
};
adobe.fn.buildTreeList = function (selectorStr) {
    if (selectorStr.length < 1) {
        return;
    }
    var myElement = $(selectorStr);
...
adobe.fn.changeRegionFooter = function (regioncode) {
    ...
    var geoArray = ["africa", "ap", "at", "au", "be_en", "be_fr", "be_nl", "bg", "br", "ca", "ca_fr", "ch_de", "ch_fr", "ch_it", "cn", "cz", "de", "dk", "eeurope", "ee", "es", "fi", "fr", "hk_en", "hk_zh", "hr", "hu", "ie", "il_en", "il_he", "in", "it", "jp", "kr", "la", "lt", "lu_de", "lu_en", "lu_fr", "lv", "mena_ar", "mena_en", "mena_fr", "mx", "nl", "no", "nz", "pl", "pt", "ro", "ru", "si", "se", "sea", "sk", "tr", "tw", "ua", "uk"];
    ...
    if (($.string(currURL).startsWith('/cfusion')) && (regioncode == 'us')) {
        newURL = "/";
        homeURL = "/";
    }
    else if (($.string(currURL).startsWith('/cfusion')) && (regioncode != 'us')) {
        newURL = "/" + regioncode + "/";
        homeURL = "/" + regioncode + "/";
    }
    ...
    $.ajax({url:newURL, type:'HEAD', error:function () {
...
adobe.fn.colorBox = function (target) {
    target = $(target);
    var cdnPrefix = adobe.http.cdnprefix();
    if ($(window).width() > 640) {
...
adobe.fn.cookies = function (target, method, cookie, value, domain, path, expire, secure) {
    var status = null, targetDiv = $('#' + target), debug = false;
    if (method == null || method == '' || cookie == null || cookie == '') {
        if (debug) {
            console.log("'method' or 'cookie' was empty. " + method + " " + cookie);
        }
        return;
    }
    switch (method) {
    ...
    function setExpire() {
        if (debug) {
            console.log("running setExpire()");
        }
        var today = new Date(), year = today.getFullYear() + 30, month = today.getMonth(), date = today.getDate();
        return new Date(year, month, date);
    }

    function showButtons(status) {
        if (status === true || status == null) {
            targetDiv.find('.CookieValid').hide();
            targetDiv.find('.CookieInvalid').show();
            targetDiv.find('.CookieOptIn').hide();
            targetDiv.find('.CookieOptOut').show();
            targetDiv.find('.CookieValue').html('');
        }
        else {
            targetDiv.find('.CookieValid').show();
            targetDiv.find('.CookieInvalid').hide();
            targetDiv.find('.CookieOptIn').show();
            targetDiv.find('.CookieOptOut').hide();
            targetDiv.find('.CookieValue').html(status);
        }
    }

    function adobeGetCookie() {
        if ($.isArray(cookie)) {
            var isCookieNull = false;
            $.each(cookie, function (index, value) {
                status = $.cookies.get(value);
                if (status == null) {
                    isCookieNull = true;
                }
            });
            showButtons(isCookieNull);
        }
        else {
            status = $.cookies.get(cookie);
            showButtons(status);
        }
        if (debug) {
            console.log("running adobeGetCookie(cookie). status = " + $.cookies.get(cookie));
        }
    }
    ...
    function adobeSetCookie() {
        if ($.cookies.test()) {
            if (debug) {
                console.log("$.cookies.test() passed " + $.cookies.test());
            }
            var cookieOptions = {domain:(domain != null && domain != '') ? domain : document.domain, path:(path != null && path != '') ? path : '/', expiresAt:(expire != null && expire != '') ? new Date(expire) : setExpire(), secure:(secure && secure != '') ? true : false};
            if (debug) {
                console.log("running adobeSetCookie(cookie).");
            }
            $.cookies.set(cookie, value, cookieOptions);
            adobeGetCookie();
        }
        else {
            status = "There was a problem testing cookies. Either you've disabled this ability, or your browser doesn't seem to support cookies. Please enable cookies and refresh your browser, or update your browser."
        }
    }

    function adobeDelCookie() {
        var cookieOptions = {domain:(domain != null && domain != '') ? domain : document.domain, path:(path != null && path != '') ? path : '/'};
        $.cookies.del(cookie, cookieOptions);
        adobeGetCookie();
        if (debug) {
            console.log("running function adobeDelCookie(cookie). status = " + $.cookies.del(cookie));
        }
    }
};
adobe.fn.embedMediaPlayer = (function ($) {
    var defaults = {defaultType:"flash"};

    function getDefaults() {
        if (typeof defaults.autoPlay == "undefined") {
            defaults.autoPlay = swfobject.getQueryParamValue('autoPlay') || "";
        }
        if (typeof defaults.allowFullScreen == "undefined") {
            defaults.allowFullScreen = swfobject.getQueryParamValue('allowFullScreen') || "";
        }
        return defaults;
    }

    function getDeepLink(id) {
        ...
        if (hash != null && hash != '') {
            ...
            if (hashArray != null && hashArray.length >= 2) {
                if (hashArray[0] == '#' + id) {
                    ...
                    out.chapterID = chapterIdDeepLink;
                }
            }
        }
        return out;
    ...
    var supportsTouch = Boolean("ontouchstart"in window), expressInstall = "/include/flash/expressInstall.swf", flashVersion = "10.1";
    return(function (id, options) {
        if (!id) {
            return;
        }
        options = $.extend({}, getDefaults(), options);
        if (!supportsTouch && options.defaultType == "flash") {
            if (options.flash) {
                swfobject.embedSWF(options.flash, id, options.width, options.height, flashVersion, expressInstall, $.extend(options.flashVars, getDeepLink(id)), options.flashParams, options.flashAttrs);
            }
        } else {
            $("#" + id).find(".FlashAltCanvas").remove();
            var bach_opts = {width:"100%"};
            if (options.height) {
                bach_opts.height = String(options.height);
            }
            if ('msgSrcError'in options) {
                bach_opts.msgSrcError = String(options.msgSrcError);
            }
            if ('msgUIError'in options) {
                bach_opts.msgUIError = String(options.msgUIError);
            }
            $(options.playlistThis).removeClass("LayoutHidden").bach_player(bach_opts);
        }
    });
})(jQuery);
adobe.fn.embedVideo = function (srcs, id, attrs) {
    function setAttr(video, user_attrs, config_attr) {
        if (user_attrs[config_attr.attr] === undefined) {
            return;
        }
        var user_attr_value = user_attrs[config_attr.attr];
        if (user_attr_value === undefined) {
            return;
        }
        if (config_attr.setEvent === undefined) {
            video.setAttribute(String(config_attr.attr), user_attr_value);
            return;
        }
        if (user_attr_value == config_attr.value || config_attr.value == "*") {
            if (config_attr.value == "*") {
                config_attr.setValue = user_attr_value;
            } else {
                video.setAttribute(String(config_attr.attr), config_attr.value);
            }
            var set_func = function (event) {
                var video = event.target;
                if (config_attr.setValue !== undefined) {
                    this[config_attr.setAttr] = config_attr.setValue;
                }
                if (typeof config_attr.setFunction == "function") {
                    config_attr.setFunction(video);
                }
                jQuery(video).unbind(config_attr.setEvent, set_func);
            };
            jQuery(video).bind(config_attr.setEvent, set_func);
        }
    }

    function replaceNode(replacement, current) {
        replacement.style.visibility = "visible";
        return current.parentNode.replaceChild(replacement, current);
    }

    function replaceNodeHandler(event) {
        jQuery(this).unbind("error adobe.event.media.nosource", replaceNodeHandler);
        replaceNode(event.data.replacement, this);
    }

    function attachSources(video, srcs) {
        var srcEl;
        for (var i = 0; i < srcs.length; i++) {
            var src = srcs[i];
            if (src.src !== undefined) {
                srcEl = window.document.createElement("source");
                srcEl.setAttribute("src", String(src.src));
                if (src.type !== undefined) {
                    srcEl.setAttribute("type", String(src.type));
                }
                video.appendChild(srcEl);
            }
        }
        if (srcEl) {
            jQuery(srcEl).bind("error", {video:video}, function (event) {
                jQuery(event.data.video).triggerHandler("adobe.event.media.nosource");
            });
        }
    }

    if (!srcs || srcs.length < 1 || !id) {
        return null;
    }
    var altElement = window.document.getElementById(String(id));
    if (altElement == null || altElement.parentNode == null) {
        return null;
    }
    var video = window.document.createElement('video'), canPlay = Boolean(video.canPlayType);
    if (!canPlay) {
        return null;
    }
    var allowed_attrs = [
        {attr:"id"},
        {attr:"audio", value:"muted", setEvent:"loadstart", setAttr:"muted", setValue:true},
        {attr:"autoplay", value:"autoplay", setEvent:"canplay", setFunction:function (video) {
            video.play();
        }},
        {attr:"controls"},
        {attr:"height"},
        ...
        {attr:"preload"},
        {attr:"src"},
        {attr:"volume", value:"*", setEvent:"loadstart", setAttr:"volume"},
        {attr:"width"}
    ];
    attrs = attrs || {};
    for (var i = 0; i < allowed_attrs.length; i++) {
        setAttr(video, attrs, allowed_attrs[i]);
    }
    jQuery(video).bind("error adobe.event.media.nosource", {current:video, replacement:altElement}, replaceNodeHandler);
    attachSources(video, srcs);
    altElement = replaceNode(video, altElement);
    return video;
};
adobe.fn.evidon = function (trigger) {
    var evidonID = $('#' + trigger), d = document, pixel, page_id, URL_SCHEME = (d.location.protocol == 'https:' ? 'https' : 'http'), CDN_URL = (URL_SCHEME == 'https' ? 'https://info.evidon.com/c/betrad/pub/' : 'http://cdn.betrad.com/pub/');
    if ((evidonID != null) && (hideEvidon != true) && isDesktop()) {
        ...
        evidonID.bind('click', function (e) {
            ...
            function appendScript(url, callback) {
                var head = d.getElementsByTagName('head')[0] || d.documentElement, loaded = false, script = d.createElement('script');
...
adobe.fn.focusHashElement = function () {
    ...
    var hasIndex = node.getAttribute("tabindex") != null;
    var hasRef = node.getAttribute("href") != null;
    if (!hasRef && !hasIndex) {
        if (node.hasAttribute("tabindex")) {
            node.setAttribute("tabindex", "0");
        } else {
            var target = $(node).find("a");
            if (target.length < 1) {
                target = $(node).find("area");
                if (target.length < 1) {
                    target = $(node).find("*[tabindex]");
                }
            }
            if (target.length > 0) {
                node = target.first();
...
adobe.fn.handleHashChangeForTreeList = function () {
    var hashId = window.location.hash;
    var myTree;
    if (hashId.length > 1) {
        myTree = $(window.document.getElementById(hashId)).closest(".ui-treeList");
    } else {
        myTree = $(".ui-treeList").first();
    }
    if (myTree.length == 0) {
        return;
    }
    var myNode = $(hashId).closest(".ui-treeList-item");
    if (myNode.length == 0) {
        myNode = myTree.find(".ui-treeList-item");
    }
    myTree.treeList('openNode', myNode);
};
adobe.fn.handleModalButton = function (event) {
    if (event.data === undefined) {
        return;
    }
    if (jQuery(window).width() > (parseInt(event.data.width) || 0)) {
        event.preventDefault();
        var dispatcher = adobe.vrbl("reflowDispatcher");
        if (dispatcher) {
...
$.getURLParameter = function (name) {
    return decodeURIComponent((location.search.match(RegExp("[?&]" + name + "=([^&]*)")) || [, ""])[1]);
}
adobe.fn.initGeorouting = function () {
    if (($('#Georouting').length) && !($.cookies.get('georouting_presented')) && ($(window).width() > 750)) {
        if ((URLParser.siteLevel == "solutions") || (URLParser.siteLevel == "products") || (URLParser.siteLevel == "solutions.html") || (URLParser.siteLevel == "index.html") || (URLParser.siteLevel == "")) {
            adobe.fn.georoutingModalSearch();
        }
    }
}
adobe.fn.georoutingModalSearch = function () {
    var referrerURL = $(document)[0].referrer;
    if (referrerURL != "") {
        var referrerDomain = referrerURL.split('/')[2];
        if ($.string(referrerDomain).endsWith('adobe.com')) {
            adobe.fn.georoutingModalIP();
        }
        else {
            if ($.string(referrerDomain).startsWith('www.')) {
                referrerDomain = referrerDomain.replace(/www\./, '');
            }
            var pageName = referrerDomain.replace(/\./g, '_');
            var fileName = "/etc/pagetables/georouting_search/" + pageName + ".modal.html";
            $.ajax({url:fileName, type:'HEAD', success:function () {
                $.cookies.set('georouting_presented', 'true', {domain:'adobe.com', hoursToLive:720});
                (function ($) {
                    adobe.fn.openModal({width:"auto", height:"auto", target:"georouting_modal-ui", href:fileName, title:""});
                })(jQuery);
            }, error:function () {
                adobe.fn.georoutingModalIP();
            }});
...
adobe.fn.georoutingModalIP = function () {
    var testCountry = $.getURLParameter("testCountry");
    if (testCountry != "") {
        tntGeocountry = testCountry;
    }
    if (typeof tntGeocountry !== 'undefined') {
        if (tntGeocountry != "") {
            var newGeocountry = tntGeocountry.replace(/[^A-Za-z]+/g, '');
            var fileName = "/etc/pagetables/georouting_ip/us_" + newGeocountry + ".modal.html";
            $.ajax({url:fileName, type:'HEAD', success:function () {
                $.cookies.set('georouting_presented', 'true', {domain:'adobe.com', hoursToLive:720});
                (function ($) {
                    adobe.fn.openModal({width:"auto", height:"auto", target:"georouting_modal-ui", href:fileName, title:""});
...
adobe.fn.gnavGetSecureCookies = function () {
    $.ajax({url:'https://verify.adobe.com/ssocheck/', dataType:'jsonp', jsonp:false, jsonpCallback:"parseSSOData"}).done(function (response) {
        if ("hasSSOCookie"in response) {
            hasSSOCookieValue = response.hasSSOCookie;
            if (hasSSOCookieValue == true) {
                if ((IMS_AUTH_CHECK_ENDPOINT) && (IMS_CLIENT_ID)) {
                    adobe.fn.gnavGetUserName();
                }
                else {
                    adobe.fn.gnavGetIMSInfo();
                }
            }
        }
    });
}
adobe.fn.gnavGetIMSInfo = function () {
    var adobe_host = window.location.hostname;
    if ((adobe_host == "www.adobe.com") || (adobe_host == "adobe.com") || (adobe_host == "forums.adobe.com") || (adobe_host == "help.adobe.com") || (adobe_host == "helpx.adobe.com") || (adobe_host == "help2.corp.adobe.com") || (adobe_host == "kb2.adobe.com") || (adobe_host == "livedocs.adobe.com") || (adobe_host == "community.adobe.com") || (adobe_host == "get.adobe.com") || (adobe_host == "get2.adobe.com") || (adobe_host == "get3.adobe.com") || (adobe_host == "store1.adobe.com") || (adobe_host == "store2.adobe.com") || (adobe_host == "store3.adobe.com")) {
        var confURL = "https://www.adobe.com/svcs/configurations/clients/adobedotcomIMS/client_aliases/adobedotcom?landscape=" + adobe_host;
    }
    else {
        var confURL = "https://www.stage.adobe.com/svcs/configurations/clients/adobedotcomIMS/client_aliases/adobedotcom?landscape=" + adobe_host;
    }
    $.ajax({url:confURL, dataType:'jsonp'}).done(function (response) {
        if (response.configurations.clients[0].client_aliases.properties) {
            var properties = {}, result = {}, propertiesRoot = response.configurations.clients[0].client_aliases.properties;
            for (var i = 0; i < propertiesRoot.length; i++) {
                properties[propertiesRoot[i].key] = propertiesRoot[i].value;
            }
            result.attributes = properties;
            if ((result.attributes.imsHost) && (result.attributes.idpHost) && (result.attributes.actualClient)) {
                $.cookies.set('gnavHostIMS', result.attributes.imsHost, {domain:'adobe.com', hoursToLive:0});
                $.cookies.set('gnavHostIDP', result.attributes.idpHost, {domain:'adobe.com', hoursToLive:0});
                $.cookies.set('gnavClientID', result.attributes.actualClient, {domain:'adobe.com', hoursToLive:0});
                IMS_AUTH_ENDPOINT = "https://" + result.attributes.imsHost + "/authorize";
                IMS_LOGOUT_ENDPOINT = "https://" + result.attributes.idpHost + "/ims/logout/v1/token";
                IMS_AUTH_CHECK_ENDPOINT = "https://" + result.attributes.idpHost + "/ims/check/v1/token";
                IMS_CLIENT_ID = result.attributes.actualClient;
                adobe.fn.gnavGetUserName();
            }
        }
    });
}
adobe.fn.gnavGetUserName = function () {
    $.ajax({url:IMS_AUTH_CHECK_ENDPOINT, data:{client_id:IMS_CLIENT_ID, scope:IMS_SCOPE}, dataType:'jsonp'}).done(function (response) {
        if ("displayName"in response) {
            var gnavUserDisplayName = response.displayName;
            $('#screenName').text(gnavUserDisplayName);
            $('#shWelcome').show();
            $('#shSignInBlock').hide();
            $('#shSignOutMobile').show();
            $('#shSignInMobile').hide();
        }
        else {
            $('#shWelcome').hide();
            $('#shSignInBlock').show();
            $('#shSignOutMobile').hide();
            $('#shSignInMobile').show();
        }
    });
}
adobe.fn.initGlobalNavAccessibility = function () {
    var timeoutInt, focusTimeoutInt, hoverTimeoutInt, tooltipTimeout, flagMouseOver = false, siteHeaderBar, siteHeaderBarDivs, siteHeaderBarItems, siteHeaderDropdownLinks, siteHeaderDropPanels, siteHeaderPanelLinks, siteHeaderLinks, searchDiv, searchInput, searchInputMobile, shInfo, infoPanel, infoPanelShadow, shSignOutLink, shWelcome, welcomePanel, welcomePanelShadow, welcomeTimeoutId, mirror, keyCodeMap = {48:"0", 49:"1", 50:"2", 51:"3", 52:"4", 53:"5", 54:"6", 55:"7", 56:"8", 57:"9", 59:";", 65:"a", 66:"b", 67:"c", 68:"d", 69:"e", 70:"f", 71:"g", 72:"h", 73:"i", 74:"j", 75:"k", 76:"l", 77:"m", 78:"n", 79:"o", 80:"p", 81:"q", 82:"r", 83:"s", 84:"t", 85:"u", 86:"v", 87:"w", 88:"x", 89:"y", 90:"z", 96:"0", 97:"1", 98:"2", 99:"3", 100:"4", 101:"5", 102:"6", 103:"7", 104:"8", 105:"9"}, init = function () {
        mirror = hasClass(document.body, "Mirror");
        siteHeader = document.getElementById("SiteHeader");
        siteHeaderBar = document.getElementById("shBar");
        siteHeaderBarDivs = siteHeaderBar.getElementsByTagName("DIV");
        siteHeaderBarItems = getElementsByClassName(siteHeaderBar, "SiteHeaderBarItem");
        siteHeaderDropdownLinks = getElementsByClassName(siteHeaderBar, "SiteHeaderDropdownLink");
        siteHeaderDropPanels = getElementsByClassName(siteHeaderBar, "SiteHeaderDropPanel");
        searchDiv = document.getElementById("site-search");
        ...
        siteHeaderLinks = siteHeaderBar.getElementsByTagName("A");
        shInfo = document.getElementById("shInfo");
        infoPanel = document.getElementById("InfoPanel");
        infoPanelShadow = document.getElementById("InfoPanelShadow");
        shSignOutLink = document.getElementById("shSignOutLink");
        shWelcome = document.getElementById("shWelcome");
        welcomePanel = document.getElementById("WelcomePanel");
        welcomePanelShadow = document.getElementById("WelcomePanelShadow");
        if (welcomePanel)welcomePanelLinks = welcomePanel.getElementsByTagName("A");
        var obj, i;
        if (navigator.platform.indexOf("Win") != -1)addClass(siteHeaderBar, "os-windows");
        siteHeader.setAttribute('role', 'navigation');
        if (searchDiv && searchInput) {
            searchDiv.setAttribute('role', 'search');
            searchInput.setAttribute('aria-label', 'Search');
        }
        if (searchDivMobile && searchInputMobile) {
            searchDivMobile.setAttribute('role', 'search');
            searchInputMobile.setAttribute('aria-label', 'Search');
        }
        for (i = 0; i < siteHeaderBarDivs.length; i++) {
            obj = siteHeaderBarDivs[i];
            obj.setAttribute('role', 'presentation');
        }
        for (i = 0; i < siteHeaderBarItems.length; i++) {
            obj = siteHeaderBarItems[i];
            obj['data-index'] = i;
        }
        for (i = 0; i < siteHeaderDropdownLinks.length; i++) {
            obj = siteHeaderDropdownLinks[i];
            obj['data-index'] = i;
        }
        for (i = 0; i < siteHeaderDropPanels.length; i++) {
            obj = siteHeaderDropPanels[i];
            obj.setAttribute('aria-hidden', 'true');
            addClass(obj, 'SiteHeaderDropPanelHidden');
            obj['data-index'] = i;
            addEvent(obj, 'mouseover', mouseOverHandler);
            addEvent(obj, 'mouseout', mouseOutHandler);
        }
        for (i = 0; i < siteHeaderLinks.length; i++) {
            obj = siteHeaderLinks[i];
            obj['data-index'] = i;
            if (!obj.getAttribute('tabindex'))
                obj.setAttribute('tabindex', (hasClass(obj.parentNode, 'SiteHeaderPanelHeader') || hasClass(obj.parentNode, 'SiteHeaderPanelRow')) ? '-1' : '0');
            if (hasClass(obj.parentNode, 'SiteHeaderDropdownLink')) {
                obj.setAttribute('title', 'Use Up/Down arrow keys for submenu');
                obj.setAttribute('aria-haspopup', 'true');
                obj.setAttribute('aria-owns', siteHeaderDropPanels[obj.parentNode['data-index']].id);
            }
            addEvent(obj, 'focus', focusHandler);
            addEvent(obj, 'blur', blurHandler);
            addEvent(obj, 'mouseover', mouseOverHandler);
            addEvent(obj, 'mouseout', mouseOutHandler);
            addEvent(obj, 'keydown', keyDownHandler);
            addEvent(obj, 'click', clickHandler);
        }
        if (shInfo && infoPanel) {
            shInfo.setAttribute('href', '#InfoPanel');
            shInfo.setAttribute('aria-describedby', 'InfoPanel');
            infoPanel.setAttribute('role', 'tooltip');
            infoPanel.style.display = "none";
            infoPanel.setAttribute('aria-hidden', 'true');
            infoPanel.setAttribute('tabindex', '-1');
            if (infoPanelShadow)infoPanelShadow.style.display = "none";
            addEvent(shInfo, "mouseover", shInfo_mouseOverHandler);
            addEvent(shInfo, "mouseout", shInfo_mouseOutHandler);
            addEvent(shInfo, "focus", shInfo_focusHandler);
            addEvent(shInfo, "blur", shInfo_blurHandler);
            addEvent(shInfo, "click", shInfo_clickHandler);
        }
        if (shWelcome && welcomePanel) {
            shWelcome.setAttribute('aria-haspopup', 'true');
            shWelcome.setAttribute('aria-owns', 'WelcomePanel');
            welcomePanel.setAttribute('role', 'menu');
            welcomePanel.style.display = "none";
            welcomePanelShadow.style.display = "none";
            addEvent(shWelcome, 'focus', shWelcome_focusHandler);
            addEvent(shWelcome, 'blur', shWelcome_blurHandler);
            addEvent(shWelcome, 'mouseover', shWelcome_mouseOverHandler);
            addEvent(shWelcome, 'mouseout', shWelcome_mouseOutHandler);
            addEvent(shWelcome, 'keydown', shWelcome_keyDownHandler);
            addEvent(shWelcome, 'click', shWelcome_clickHandler);
            addEvent(shWelcome, 'contextmenu', shWelcome_contextMenuHandler);
            addEvent(welcomePanel, 'mouseover', shWelcome_mouseOverHandler);
            addEvent(welcomePanel, 'mouseout', shWelcome_mouseOutHandler);
            addEvent(shSignOutLink, 'click', shWelcome_hidePanel);
            for (i = 0; i < welcomePanelLinks.length; i++) {
                obj = welcomePanelLinks[i];
                obj.setAttribute('role', 'menuitem');
                obj['data-index'] = i;
                if (!obj.getAttribute('tabindex'))
                    obj.setAttribute('tabindex', '-1');
                addEvent(obj, 'focus', shWelcome_focusHandler);
                addEvent(obj, 'blur', shWelcome_blurHandler);
                addEvent(obj, 'mouseover', shWelcome_mouseOverHandler);
                addEvent(obj, 'mouseout', shWelcome_mouseOutHandler);
                addEvent(obj, 'keydown', shWelcome_keyDownHandler);
                addEvent(obj, 'contextmenu', shWelcome_contextMenuHandler);
            }
        }
    }, shWelcome_contextMenuHandler = function (event) {
        event.preventDefault();
        event.stopPropagation();
        return false;
    }, shWelcome_mouseOverHandler = function (event) {
        clearTimeout(welcomeTimeoutId);
        clearToolTip();
        hideAndShowSubmenus(this || event.target, 'closeall');
        shWelcome_showPanel();
    }, shWelcome_mouseOutHandler = function (event) {
        clearTimeout(welcomeTimeoutId);
        if (document.activeElement != shWelcome) {
            var welcomePanelLink, i;
            for (i = 0; i < welcomePanelLinks.length; i++) {
                if (document.activeElement == welcomePanelLinks[i])
                    return true;
            }
            welcomeTimeoutId = setTimeout(shWelcome_hidePanel, 450);
        }
    }, shWelcome_focusHandler = function (event) {
        clearTimeout(welcomeTimeoutId);
        clearToolTip();
        hideAndShowSubmenus(this || event.target, 'closeall');
        shWelcome_showPanel();
    }, shWelcome_blurHandler = function (event) {
        clearTimeout(welcomeTimeoutId);
        if (document.activeElement != shWelcome) {
            var welcomePanelLink, i;
            for (i = 0; i < welcomePanelLinks.length; i++) {
    ...
    }, shWelcome_keyDownHandler = function (event) {
        var node, links, start, j, a, label, candidate, found = false, beforeLink = true;
        switch (event.keyCode) {
            case 38:
            {
                event.preventDefault();
                if (this == shWelcome) {
                    ...
                    node = nextTabbableElement(this, true, true);
                    if (node) {
                        node.focus();
                        if (node == shWelcome) {
                            shWelcome_hidePanel();
            ...
            case 40:
            {
                event.preventDefault();
                shWelcome_showPanel();
                node = nextTabbableElement(this, false, true);
                if (node) {
            ...
            default:
            {
                ...
                for (j = start + 1; j < links.length; j++) {
                    a = links[j];
                    label = a.innerText || a.textContent;
                    if (label.substring(0, 1).toLowerCase() == keyCodeMap[event.keyCode]) {
                        a.focus();
                        found = true;
                        break;
                    }
    ...
    }, shWelcome_clickHandler = function (event) {
        event.preventDefault();
        event.stopPropagation();
        clearToolTip();
        hideAndShowSubmenus(this || event.target, 'closeall');
        ...
        welcomePanelLinks[0].focus();
        return false;
    }, shWelcome_hidePanel = function () {
        clearTimeout(welcomeTimeoutId);
        welcomePanel.style.display = 'none';
        welcomePanel.setAttribute('aria-hidden', 'true');
        welcomePanelShadow.style.display = 'none';
        removeEvent(document, 'keydown', shWelcome_escapeKeyHandler);
    }, shWelcome_showPanel = function () {
        clearTimeout(welcomeTimeoutId);
        welcomePanel.style.display = 'block';
        welcomePanel.setAttribute('aria-hidden', 'false');
        welcomePanelShadow.style.width = (welcomePanel.clientWidth) + 'px';
        welcomePanelShadow.style.height = (welcomePanel.clientHeight) + 'px';
        welcomePanelShadow.style.display = 'block';
        addEvent(document, 'keydown', shWelcome_escapeKeyHandler);
    }, shWelcome_escapeKeyHandler = function (event) {
        if (event.keyCode == 27) {
            if (document.activeElement != shWelcome) {
                var welcomePanelLink, i;
                for (i = 0; i < welcomePanelLinks.length; i++) {
    ...
    }, shInfo_mouseOverHandler = function (event) {
        clearToolTip();
        hideAndShowSubmenus(this || event.target, 'closeall');
        shInfo_showPanel();
    }, shInfo_mouseOutHandler = function (event) {
        shInfo_hidePanel();
    }, shInfo_focusHandler = function (event) {
        clearToolTip();
        hideAndShowSubmenus(this || event.target, 'closeall');
        shInfo_showPanel();
    }, shInfo_blurHandler = function (event) {
        shInfo_hidePanel();
    }, shInfo_clickHandler = function (event) {
        clearToolTip();
        hideAndShowSubmenus(this || event.target, 'closeall');
        shInfo_showPanel();
        event.preventDefault();
        return false;
    }, shInfo_hidePanel = function () {
        infoPanel.style.display = 'none';
        infoPanel.setAttribute('aria-hidden', 'true');
        infoPanelShadow.style.display = 'none';
        removeEvent(document, 'keydown', shInfo_escapeKeyHandler);
    }, shInfo_showPanel = function () {
        infoPanel.style.display = 'block';
        infoPanel.setAttribute('aria-hidden', 'false');
        infoPanelShadow.style.height = (infoPanel.clientHeight) + 'px';
        infoPanelShadow.style.display = 'block';
        addEvent(document, 'keydown', shInfo_escapeKeyHandler);
    }, shInfo_escapeKeyHandler = function (event) {
        if (event.keyCode == 27)
            shInfo_hidePanel();
        ...
        clearTimeout(focusTimeoutInt);
        clearToolTip();
        var scope = this, eventType = event.type || 'focus';
        ...
        if (hasClass(scope.parentNode, "SiteHeaderDropdownLink")) {
            focusTimeoutInt = setTimeout(function () {
                clearTimeout(focusTimeoutInt);
                hideAndShowSubmenus(scope, eventType);
            }, 100);
            return;
        }
        if (hasClass(scope.parentNode.parentNode, "SiteHeaderPanelLink")) {
        ...
        clearTimeout(focusTimeoutInt);
        clearToolTip();
        var scope = this, eventType = event.type || 'blur';
        if (hasClass(scope.parentNode, "SiteHeaderDropdownLink")) {
            focusTimeoutInt = setTimeout(function () {
                clearTimeout(focusTimeoutInt);
                hideAndShowSubmenus(scope, eventType);
        ...
        clearTimeout(hoverTimeoutInt);
        clearToolTip();
        var scope = this, eventType = event.type || 'mouseover';
        hideAndShowSubmenus(scope, eventType);
        if (hasClass(scope.parentNode.parentNode, "SiteHeaderPanelLink")) {
            addClass(scope.parentNode.parentNode, "SiteHeaderPanelLinkHover");
        }
        ...
        clearTimeout(hoverTimeoutInt);
        clearToolTip();
        var scope = this, eventType = event.type || 'mouseout';
        if (hasClass(scope.parentNode.parentNode, "SiteHeaderPanelLink") && document.activeElement != scope) {
            removeClass(scope.parentNode.parentNode, "SiteHeaderPanelLinkHover");
        }
        hoverTimeoutInt = setTimeout(function () {
            clearTimeout(hoverTimeoutInt);
            hideAndShowSubmenus(scope, eventType);
        }, 500);
    }, keyDownHandler = function (event) {
        var siteHeaderDropdownLink = getClosestSiteHeaderDropdownLink(this), node, links, i;
        switch (event.keyCode) {
            case 9:
            {
                flagKeyboardNavigation(true);
            ...
            {
                event.preventDefault();
                if ((event.keyCode == 37 && !mirror) || (event.keyCode == 39 && mirror)) {
                    flagKeyboardNavigation(true);
                    if (siteHeaderDropdownLink && siteHeaderDropdownLink != this.parentNode) {
                        siteHeaderDropdownLink.getElementsByTagName("A")[0].focus();
                        addClass(siteHeaderDropPanels[siteHeaderDropdownLink['data-index']], 'SiteHeaderDropPanelHidden');
                        siteHeaderDropPanels[siteHeaderDropdownLink['data-index']].setAttribute('aria-hidden', 'true');
                    }
                    else {
                        node = previousElementSibling(this.parentNode == siteHeaderDropdownLink ? this.parentNode : this);
                        if (node) {
                            if (node.nodeName.toLowerCase() == "a") {
                                node.focus();
                            }
                            else if (hasClass(node, 'SiteHeaderDropdownLink')) {
                                firstElementChild(node).focus();
                            }
                        } else {
                            i = siteHeaderBarItems.length;
                            while (i > 0) {
                                i--;
                                node = siteHeaderBarItems[i];
                    ...
                    flagKeyboardNavigation(true);
                    node = nextElementSibling(siteHeaderDropdownLink || this);
                    if (node) {
                        if (node.nodeName.toLowerCase() == "a" && focusable(node)) {
                            node.focus();
                        }
                            ...
                            i = 0;
                            while (i < siteHeaderBarItems.length) {
                                node = siteHeaderBarItems[i];
                                if (node.nodeName.toLowerCase() == "a" && focusable(node)) {
                                    node.focus();
                                    break;
            ...
            case 13:
            {
                ...
                if (siteHeaderDropdownLink) {
                    if (hasClass(siteHeaderDropPanels[siteHeaderDropdownLink['data-index']], 'SiteHeaderDropPanelHidden')) {
                        removeClass(siteHeaderDropPanels[siteHeaderDropdownLink['data-index']], 'SiteHeaderDropPanelHidden');
                        siteHeaderDropPanels[siteHeaderDropdownLink['data-index']].setAttribute('aria-hidden', 'false');
                    }
                    else {
                        window.location = this.href;
                    }
            ...
            default:
            {
                var start, j, a, label, candidate, found = false, beforeLink = true;
                if (!siteHeaderDropdownLink)return;
                links = siteHeaderDropdownLink.getElementsByTagName("A");
                var start = 0;
                for (j = 0; j < links.length; j++) {
                    a = links[j];
                    if (a == this) {
        ...
        event.preventDefault();
        event.stopPropagation();
        if (event.target && event.target.parentNode && hasClass(event.target.parentNode, "SiteHeaderDropdownLink") && hasClass(siteHeaderDropPanels[event.target.parentNode['data-index']], 'SiteHeaderDropPanelHidden')) {
            removeClass(siteHeaderDropPanels[event.target.parentNode['data-index']], 'SiteHeaderDropPanelHidden');
            siteHeaderDropPanels[event.target.parentNode['data-index']].setAttribute('aria-hidden', 'false');
            flagKeyboardNavigation(true);
            return false;
        } else {
            if (event.target) {
                if (hasClass(event.target, "SiteHeaderPanelLinkDescription")) {
                ...
                if (hasClass(event.srcElement, "SiteHeaderPanelLinkDescription")) {
                    window.location = event.srcElement.parentNode;
                }
                else {
        ...
        flagKeyboardNavigation(false);
    }, flagKeyboardNavigation = function (bool) {
        if (bool) {
            addClass(siteHeaderBar, "SiteHeaderBarKeyboardNavigation");
            addEvent(siteHeaderBar, "mousemove", mouseMoveHandler);
        } else {
            removeClass(siteHeaderBar, "SiteHeaderBarKeyboardNavigation");
            removeEvent(siteHeaderBar, "mousemove", mouseMoveHandler);
        }
    }, getClosestSiteHeaderDropdownLink = function (element) {
        var siteHeaderBar = document.body, p = element.parentNode;
        while (p && p != siteHeaderBar) {
        ...
        var regExp = new RegExp(new RegExp("(\\s*|^)" + className + "(\\s*|$)"));
        return element.className && regExp.test(element.className);
    }, addClass = function (element, className) {
        if (!hasClass(element, className))element.className += " " + className;
    }, removeClass = function (element, className) {
        if (hasClass(element, className)) {
            var regExp = new RegExp("^" + className + "(\\s*|$)|\\s*" + className);
            element.className = element.className.replace(regExp, "");
        }
    }, getElementsByClassName = function (element, className) {
        var a = [], i, ele, elements = element.getElementsByTagName("*");
        ...
        var nodeName = element.nodeName.toLowerCase(), map, mapName, images, img, i;
        if ("area" === nodeName) {
            ...
            for (i = 0; i < images.length; i++) {
                img = images[i];
                if (img.getAttribute("usemap") === "#" + mapName && visible(img))
                    return true;
            }
            return false;
        }
        ...
        var tabIndex = element.getAttribute("tabindex") ? parseInt(element.getAttribute("tabindex")) : NaN, isTabIndexNaN = isNaN(tabIndex);
        return(isTabIndexNaN || tabIndex >= 0) && focusable(element, !isTabIndexNaN);
    }, nextTabbableElement = function (element, shiftKey, includeFocusable) {
        var i, allElements = document.body.getElementsByTagName("*"), afterElement = false, candidate, current;
        for (i = 0; i < allElements.length; i++) {
            current = allElements.item(i);
            if (current == element) {
                afterElement = true;
                if (candidate && shiftKey)return candidate;
            }
    ...
    }, addToolTip = function (element, message) {
        clearTimeout(tooltipTimeout);
        var title = message || element.getAttribute("title");
        if (title) {
            element.removeAttribute("title");
            var tooltip = document.createElement("div"), tooltipId = "tooltip_" + element.getAttribute("id"), tooltipSpan = document.createElement("span"), tooltipText = document.createTextNode(title);
            tooltip.setAttribute("id", tooltipId);
            tooltip.setAttribute("role", "tooltip");
            tooltip.setAttribute("tabindex", "-1");
            addClass(tooltip, "SiteHeaderBarTooltip");
        ...
        clearTimeout(tooltipTimeout);
        var tooltips = getElementsByClassName(document, "SiteHeaderBarTooltip"), j;
        for (j = 0; j < tooltips.length; j++) {
            var tooltip = tooltips[j], tooltipId = tooltip.getAttribute("id");
            element = document.getElementById(tooltipId.substring(8));
            if (element) {
                element.setAttribute("title", tooltip.firstChild.firstChild.nodeValue);
        ...
        return(node.nodeType == 8) || ((node.nodeType == 3) && isAllWhitespace(node));
    }, previousElementSibling = function (sibling) {
        while (sibling = sibling.previousSibling) {
            if (!isIgnorable(sibling))return sibling;
        }
        return null;
    }, nextElementSibling = function (sibling) {
        while (sibling = sibling.nextSibling) {
            if (!isIgnorable(sibling))return sibling;
        }
        return null;
    }, lastElementChild = function (parentNode) {
        ...
        return null;
    }, firstElementChild = function (parentNode) {
        ...
        if (element.addEventListener) {
            ...
            if (!handler['data-guid'])handler['data-guid'] = addEvent.guid++;
            if (!element['data-events'])element['data-events'] = {};
            var handlers = element['data-events'][type];
            if (!handlers) {
                handlers = element['data-events'][type] = {};
                if (element['on' + type])handlers[0] = element['on' + type];
                element['on' + type] = handleEvent;
            }
            handlers[handler['data-guid']] = handler;
        }
    }, removeEvent = function (element, type, handler) {
        if (element.removeEventListener) {
            element.removeEventListener(type, handler, false);
        } else if (element['data-events'] && element['data-events'][type] && handler['data-guid'])
            delete element['data-events'][type][handler['data-guid']];
    }, handleEvent = function (event) {
        event = event || fixEvent(window.event);
        var returnValue = true;
        var handlers = this['data-events'][event.type];
        for (var i in handlers) {
            if (!Object.prototype[i]) {
                this['data-handler'] = handlers[i];
                if (this['data-handler'](event) === false)returnValue = false;
            }
        }
        if (this['data-handler'])this['data-handler'] = null;
        return returnValue;
...
adobe.fn.initGlobalNavAccessibility();
adobe.fn.initReflowComponent = function (dispatcher, headElement, layoutName, cssFile) {
    var css;
    var dispatcher = dispatcher, head = headElement, layoutName = layoutName, cssFile = cssFile;
    handleLayoutEvent(null, dispatcher.getLayoutEvent(layoutName));
    function handleLayoutEvent(event, layoutEvent) {
        if (layoutEvent.name == layoutName) {
            if (!css) {
                css = new adobe.dom.CSSStyleSheet(cssFile);
            }
...
adobe.fn.initRssFeed = function (id, feedURL) {
    ...
    if ($('#' + id + 'feedTitle').length != 0) {
        $.getFeed({url:feedURL, success:function (feed) {
            $('#' + id + 'feedTitle').html(feed.title);
            $('#' + id + 'feedLink .RouterHeadingLink').attr('href', feed.link);
            var arr = feed.items;
            var max = 140;
            var textToInsert = [];
            var i = 0;
            ...
            for (var a = 0; a < length; a += 1) {
                ...
                textToInsert[i++] = '<tr>';
                textToInsert[i++] = '<th class="TableCell TableNumber">' + date + '</th>';
                textToInsert[i++] = '<td>';
                textToInsert[i++] = '<a target="_blank" href="' + item.link + '">' + cuttext(item.title, max) + '</a>';
                textToInsert[i++] = '</td>';
                textToInsert[i++] = '</tr>';
            }
...
adobe.fn.slider = function (e) {
    var scrollPane = $("#" + e + " .ui-slider-pane"), scrollContent = $("#" + e + " .ui-slider-content"), scrollContentItems = $("#" + e + " .ui-slider-content .ui-slider-item"), scrollContentImages = $("#" + e + " .ui-slider-content .ui-slider-item img");
    var scrollItemsWidth = 20;
    scrollContentItems.each(function () {
        scrollItemsWidth += $(this).innerWidth();
    });
    scrollContent.css('width', scrollItemsWidth);
    var scrollbar = $("#" + e + " .ui-slider-bar").slider({slide:function (event, ui) {
    ...
    var handleHelper = scrollbar.find(".ui-slider-handle").append("<span class='ui-icon ui-icon-grip-dotted-vertical ui-icon-small'></span>").wrap("<div class='ui-handle-helper-parent'></div>").parent();
    scrollContentImages.each(function () {
        $(this).addClass('LayoutHAlignMiddle');
    });
...
adobe.fn.openModal = function (config) {
    ...
    var currentWindowSize = {width:0, height:0};

    function createActiveLayoutHandler(element, dimensionKey, position, failSafePosition, dimensionBuffer) {
        var dimensionValue = element.dialog("option", dimensionKey);
        var isAuto = dimensionValue == "auto";
        var handler = function () {
            var viewSize = $(window)[dimensionKey]();
            if (isAuto) {
                element.dialog("option", dimensionKey, dimensionValue);
            }
            var tooBig = element.dialog("widget")[dimensionKey]() > viewSize;
            if (isAuto && tooBig) {
                element.dialog("option", dimensionKey, viewSize - dimensionBuffer);
            }
            if (tooBig) {
                element.dialog("option", "position", failSafePosition);
                var v = parseInt(element.dialog("widget").css(failSafePosition));
            }
            else {
                element.dialog("option", "position", position);
            }
        };
        if (dimensionValue == "auto") {
            handler();
    ...
    function createOpenModalHandler(config) {
        var modal = $("#" + config.target), marginWidth = 40, marginHeight = 62;
        modal.dialog({modal:true, title:config.title || "", height:getValidDimension(config.height, marginHeight), width:getValidDimension(config.width, marginWidth), autoOpen:false, resizable:false, draggable:false, closeOnClick:true, open:function () {
            ...
            $(window).bind("resize", {dimension:"height", handler:createActiveLayoutHandler(modal, 'height', 'center', 'center', marginHeight)}, handleWindowResize);
            $(window).bind("resize", {dimension:"width", handler:createActiveLayoutHandler(modal, 'width', 'center', 'center', marginWidth)}, handleWindowResize);
        ...
        function embedSWF() {
            var hrefValue = config.href;
            var hrefValues = new Array();
            hrefValues = hrefValue.split('/');
            var flashvars = {fileID:hrefValues[5], context:hrefValues[4], embeded:"true", autoPlay:"true"};
            var params = {menu:"false", wmode:"transparent", allowfullscreen:"true"};
            var id = {'class':'ui-dialog-content ui-widget-content', 'name':'ui-dialog-content ui-widget-content'};
            swfobject.embedSWF("http://images.tv.adobe.com/cdn/swf/player.swf", config.target, config.width, config.height, "9.0.0", "expressInstall.swf", flashvars, params, id);
        }
                ...
                if (status == "error" && external == "undefined") {
                    ...
                    if ($.browser.msie && parseFloat($.browser.version) <= 6.9) {
                        if ($("#" + config.target + " img").length != 0) {
                            ...
                            var combinedXObjects = (newImg.width + marginWidth) - 20;
                            var combinedYObjects = newImg.height + marginHeight + 11;
                            modal.dialog('option', 'width', combinedXObjects);
                            modal.dialog('option', 'height', combinedYObjects);
                            modal.dialog('open');
                        }
...
adobe.fn.pageFilter = function (e) {
    var menuBarID = "#" + e, menuBar = $(menuBarID), menuControlID = "MenuControl", menuControl = $('#' + menuControlID), menuShow = $(menuBarID + " .MenuShow"), menuButton = $(menuBarID + ' .MenuDropButton');
    if (menuControl[0] == null) {
        $('<div/>', {id:menuControlID}).appendTo('body');
        menuControl = $('#' + menuControlID);
    }
    var menuDropWidth = $(menuBarID + ' .MenuDrop').width(), menuPanelWidth = $(menuBarID + ' .MenuDropPanel').width(), menuModifier = 1, menuWidth;
    ...
    $(menuBarID + ' .MenuDrop').css('width', menuWidth);
    $(menuBarID + ' .MenuDropPanel').css('width', menuWidth);
    menuShow.each(function () {
        var targetShow = $(this).attr('rel'), moreLink = $(this).attr('href');
        $(this).bind('click', function (event) {
            event.preventDefault();
            var menuToggle = $('.MenuToggle');
            menuToggle.each(function () {
                if ($(this).attr('id') != '#' + targetShow) {
                    menuToggle.addClass('UIHide');
                }
            });
            $('#' + targetShow).removeClass('UIHide');
            var menuSelected = $(menuBarID + ' .MenuDropItemSelected').html();
            menuBar.parents('td').siblings('.MenuMultiPanel').find('a.RouterLink').attr('href', moreLink);
            menuBar.find('.MenuButton a').html(menuSelected);
            $('.MenuDropPanel').slideUp('fast').removeClass('MenuShow');
            menuButton.removeClass('LayoutFlipV');
            menuControl.removeClass('MenuClose');
        });
    });
    menuBar.find("div[class*='MenuDropName']").bind('click', function () {
        ...
        if (menuPanel.hasClass('MenuShow')) {
            menuPanel.slideUp('fast').removeClass('MenuShow');
            menuControl.removeClass('MenuClose');
            menuButton.removeClass('LayoutFlipV');
        }
        else {
            menuPanel.slideDown('fast').addClass('MenuShow');
            menuControl.addClass('MenuClose');
            menuButton.addClass('LayoutFlipV');
            menuControl.unbind('click');
            menuControl.bind('click', function () {
                $(menuBarID + ' .MenuDropPanel').slideUp('fast').removeClass('MenuShow');
                menuButton.removeClass('LayoutFlipV');
                menuControl.removeClass('MenuClose');
            });
            $('body').unbind('keyup');
...
adobe.fn.ratings = function (target, xml) {
    var body = $('body'), rating, num, cdnPrefix = adobe.http.cdnprefix(), xmlInProgress = body.data('XmlDataInProgress'), xmlDataDone = body.data('XmlDataDone');
    if (xmlDataDone == null) {
        if (xmlInProgress == null) {
            body.data('XmlDataInProgress', 'true');
            var head = $("head")[0], ratingsCss = new adobe.dom.CSSStyleSheet(cdnPrefix + "/include/style/ratings.css");
            ratingsCss.enable(head);
            body.bind('XmlDataChange', function (e, id, rating) {
                $('#' + id).addClass('StarRatings-' + rating);
            });
            $.get(xml, function (dataObject) {
                $(dataObject).find("Product").each(function () {
                    rating = $(this).find('ReviewStatistics').find('AverageOverallRating').text(), num = new Number(rating);
                    rating = num.toFixed(1);
                    rating = rating.replace('.', '_');
                    body.data($(this).attr('id'), {productRating:rating});
                    body.data('XmlDataDone', 'true');
                    body.trigger('XmlDataChange', [$(this).attr('id'), rating]);
                });
            });
        }
    }
...
adobe.fn.resizeObjectElement = function (objectId, width, height) {
    function setAtt(name, value) {
        ...
        if (e.hasAttribute("style")) {
            e.style[name] = value + "px";
        }
    }

    var h = parseInt(height), w = parseInt(width), e = window.document.getElementById(objectId.toString());
    if (e == null) {
        return;
    }
    setAtt("height", h);
...
adobe.fn.toggle = function (click, toggle, toggle2, slide) {
    var toggleId = $("#" + toggle), clickId = $("#" + click);
    toggleId.attr('aria-hidden', 'true');
    if (toggle2 != undefined && toggle2 != '') {
        var toggleId2 = $("#" + toggle2);
        toggleId2.attr('aria-hidden', 'true');
    }
    clickId.bind('click', function (e) {
        e.preventDefault();
        (toggleId.attr('aria-hidden') == 'true') ? toggleId.attr('aria-hidden', 'false') : toggleId.attr('aria-hidden', 'true');
        (slide != undefined && slide != '') ? toggleId.slideToggle(slide) : toggleId.toggle();
        if (toggle2 != undefined && toggle2 != '') {
            (toggleId2.attr('aria-hidden') == 'true') ? toggleId2.attr('aria-hidden', 'false') : toggleId2.attr('aria-hidden', 'true');
            (slide != undefined && slide != '') ? toggleId2.slideToggle(slide) : toggleId2.toggle();
        }
    });
};
adobe.http = {};
adobe.http.cdnprefix = function () {
    var cdnPrefix, hostProtocol = (document.location.protocol == 'https:' ? 'https' : 'http'), host = URLParser.host;
    switch (host) {
        case"www.adobe.com":
            cdnPrefix = hostProtocol + "://wwwimages.adobe.com/www.adobe.com";
            break;
        case"adobe.com":
            cdnPrefix = hostProtocol + "://wwwimages.adobe.com/www.adobe.com";
            break;
        case"www.stage.adobe.com":
            cdnPrefix = hostProtocol + "://wwwimages.stage.adobe.com/www.stage.adobe.com";
            break;
        default:
            cdnPrefix = "";
            break;
...
adobe.http.getCookies = function (cookie) {
    function trim(s) {
        return((s === undefined) ? "" : String(s).replace(/^\s*/, "").replace(/\s*$/, ""));
    }
    ...
    if (typeof cookie == 'string') {
        ...
        for (var i = 0, l = cookies.length; i < l; i++) {
            ...
            var name = trim(pieces[0]), content = trim(pieces[1]);
            o[name] = new adobe.http.Cookie(name, content);
        }
    }
    return o;
};
adobe.http.Cookie = function (name, content) {
    this.setName(name);
    this.setContent(content);
    return this;
};
adobe.http.Cookie.prototype = {setName:function (name) {
    this.name = String(name);
    return this;
}, setContent:function (content) {
    this.content = (typeof content == 'string') ? content : "";
    return this;
}, setExpires:function (milliseconds) {
    this.expires = isNaN(milliseconds) ? 0 : milliseconds;
    return this;
}, setSecure:function (secure) {
    this.secure = Boolean(secure);
    return this;
}, setDomain:function (domain) {
    this.domain = encodeURI(String(domain));
    return this;
}, setPath:function (path) {
    this.path = encodeURI(String(path));
    return this;
}, destroy:function () {
    this.content = "";
    this.setExpires(-86400000);
    return this;
}, toString:function () {
    var cookie = this.name + "=" + this.content;
    if (!isNaN(this.expires)) {
        var date = new Date();
...
adobe.http.Cookie.is = function (func) {
    return func !== undefined && adobe.http.Cookie.prototype.isPrototypeOf(func);
};
adobe.ui = {};
adobe.ui.CartFacade = function (overlay_id, flash_id) {
    this.overlayElement = null;
    this.distributionMethod = "";
    this.distributionMethodDisplayed = "";
    this.storeTypeDisplayed = "";
    this.categoryPath = "";
    this.categoryPathDisplayed = "";
    this.contextPath = "";
    this._domain = "";
    this.defaultStore = "";
    this.defaultCountry = "";
    ...
    this.storeRegion = "";
    this.storeRegionCookie = new adobe.http.Cookie("storeregion").setPath('/').setExpires(86400002 * 365);
    this.productName = "";
    this.emailTrackingId = "";
    this.storeType = "";
    this.o_id = overlay_id;
    this.f_id = flash_id;
    this.flashObject = null;
};
adobe.ui.CartFacade.prototype = {closeCartOverlay:function () {
    if (!this.overlayElement) {
        return;
    }
    this.overlayElement.dialog('close');
}, getDetectedFlashVersion:function () {
    if (!this.detectedFlashVersion) {
        var plVersion = swfobject.getFlashPlayerVersion();
        this.detectedFlashVersion = plVersion.major + "." + plVersion.minor + "." + plVersion.release;
    }
    ...
    if (this.storeRegion) {
        ...
        var cookie = adobe.http.getCookies(window.document.cookie).storeregion;
        if (cookie) {
    ...
    var supported = this.isFlashVersionSupported();
    supported = supported && !(window.opera && (/^Win/.test(window.navigator.platform)));
    return supported;
    ...
    for (var i = 0; i < playerVersion.length; i++) {
        var current = playerVersion[i] || 0, required = requiredVersion[i] || 0;
        if (current > required) {
            break;
    ...
    var v = String(version).split("."), r = [];
    for (var i = 0, l = v.length; i < l; i++) {
        r.push(parseFloat(v[i]));
    }
    return r;
}, openCartOverlay:function () {
    if ((this.overlayElement == null) || (this.distributionMethod != this.distributionMethodDisplayed) || (this.storeType != this.storeTypeDisplayed || this.categoryPath != this.categoryPathDisplayed)) {
        var vars = {categorypath:this.categoryPath, contextpath:this.contextPath, defaultstore:this.defaultStore, defaultcountry:this.defaultCountry, distmethod:this.distributionMethod, flashversion:this.getDetectedFlashVersion(), omnitureproductname:this.productName, promoid:this.emailTrackingId, storeregion:this.getStoreRegion(), storetype:this.storeType}, params = {allowScriptAccess:"sameDomain", id:this.f_id}, attrs = {id:this.f_id, name:this.f_id, allowScriptAccess:"sameDomain"}, _this = this;
        var overlayElement = document.getElementById(this.o_id);
        if (!overlayElement) {
            var body = document.getElementsByTagName("body")[0];
            var div = document.createElement("div");
            div.setAttribute("id", this.o_id);
            overlayElement = body.appendChild(div);
        }
        this.overlayElement = $("#" + this.o_id);
        var flashObject = document.getElementById(this.f_id);
        if (!flashObject) {
            flashObject = overlayElement.appendChild(document.createElement("div"));
            flashObject.setAttribute("id", this.f_id);
        }
        this.overlayElement.dialog({modal:true, autoOpen:false, dialogClass:"CartDialog", resizable:false, draggable:false, width:850, height:535, closeOnClick:false, open:function () {
            swfobject.replaceSWF(_this.contextPath + "/swf/store/flex/StatelessStore.swf", _this.f_id, "850", "535", _this.requiredFlashVersion, "", vars, params, attrs, function (event) {
                if (event.success) {
                    _this.flashObject = document.getElementById(_this.f_id);
                }
            });
        }});
        this.distributionMethodDisplayed = this.distributionMethod;
        this.storeTypeDisplayed = this.storeType;
        this.categoryPathDisplayed = this.categoryPath;
    }
    this.overlayElement.dialog('open');
}, resizeCartOverlay:function (width, height) {
    if (!this.overlayElement) {
        return;
    }
    if (this.flashObject) {
        ...
        this.flashObject.sizeChanged(1);
    }
    this.overlayElement.dialog("option", {height:height, width:width, position:"center"});
}, setCategoryPath:function (catpath) {
    this.categoryPath = String(catpath);
}, setContextPath:function (path) {
    this.contextPath = String(path);
    ...
    this.storeRegionCookie.setDomain(this._domain);
}, setEmailTrackingId:function (prmid) {
    this.emailTrackingId = String(prmid);
}, setProductName:function (productname) {
    this.productName = String(productname);
}, setRequiredFlashVersion:function (version) {
    this.requiredFlashVersion = String(version);
}, setStoreRegion:function (region) {
    this.storeRegion = String(region);
    window.document.cookie = this.storeRegionCookie.setContent(this.storeRegion).toString();
...
adobe.ui.formDestroy = function (target) {
    target = $('#' + target);
    if (target.html() != '') {
        var targetDestroy = target.find('select').attr('id');
        $('#' + targetDestroy).selectBox('destroy');
    }
};
adobe.ui.formLoad = function (formID, target, dir, ext) {
    ...
    $(target).empty();
    if (target == "#MarketSegment-target") {
        $('#MemberType-target').empty();
    }
    var formIDValue = $("#" + formID + " option:selected").val();
    if (formIDValue !== undefined && formIDValue != '') {
...
adobe.ui.selectSwitch = function (e, toggle) {
    ...
    $("#" + e).change(function () {
        $("#" + e + " option:selected").each(function () {
            if (target != '') {
                $('#' + target).addClass("LayoutMinV LayoutMask").removeClass("selectDisabled");
                $('#' + target + ' select').selectBox('value', "");
                $('#' + target + " .selectDisabled").removeClass("LayoutMinV LayoutMask");
            }
            target = $(this).val();
            if (target != '') {
                $('#' + target).removeClass("LayoutMinV LayoutMask").addClass("selectDisabled");
                $('#' + target + ' select').selectBox('value', "");
                $('#' + target + " .selectDisabled").addClass("LayoutMinV LayoutMask");
            }
        });
...
adobe.ui.formValidate = function (target, thanks, meta, attr) {
    meta = (meta == null) ? true : meta;
    var targetForm = $('#' + target), thanksDiv = $('#' + thanks), cdnPrefix = adobe.http.cdnprefix();
    if (target !== undefined) {
        targetForm.attr('novalidate', 'novalidate');
        $.getScript(cdnPrefix + '/include/script/jquery/plugins/OnDemand/jquery.form.js');
        targetForm.find(':submit').before('<div id="SubmitStatus" />');
        var SubmitStatus = $('#SubmitStatus');
        var validateOptions = {wrapper:"div class='LayoutRow'", errorClass:"FormError", errorPlacement:function (error, element) {
            var validator = $.data(targetForm[0], 'validator'), elementIdOrName = validator.idOrName(element[0]), elementLabel = $('label[for="' + elementIdOrName + '"]').eq(0), isMacOriOS = navigator.userAgent.match(/Mac|iPhone|iPad/i) != null;
            if (elementLabel && !elementLabel.attr('id')) {
                elementLabel.attr('id', elementIdOrName + 'Label');
            }
            element.data('aria-labelledby', (!elementLabel || isMacOriOS) ? elementIdOrName + this.errorClass : elementLabel.attr('id') + ' ' + elementIdOrName + this.errorClass);
            error.insertAfter(element.attr({'aria-invalid':'true', 'aria-labelledby':element.data('aria-labelledby')})).find(this.errorElement).attr({'id':elementIdOrName + this.errorClass});
        }, showErrors:function (errorMap, errorList) {
            for (var i = 0; errorList[i]; i++) {
                var error = errorList[i], element = $(error.element);
                element.attr({'aria-invalid':'true', 'aria-labelledby':element.data('aria-labelledby')});
            }
            this.defaultShowErrors();
        }, unhighlight:function (element, errorClass, validClass) {
            if (element.type === 'radio') {
                this.findByName(element.name).removeClass(errorClass).addClass(validClass).removeAttrs("aria-invalid aria-labelledby");
            } else {
                $(element).removeClass(errorClass).addClass(validClass).removeAttrs("aria-invalid aria-labelledby");
            }
        }};
        $.getScript(cdnPrefix + '/include/script/jquery/plugins/OnDemand/jquery.validate.js').done(function () {
            if (meta) {
                $.getScript(cdnPrefix + '/include/script/jquery/plugins/OnDemand/jquery.metadata.js').done(function () {
                    attr = (attr == null) ? 'data-validate' : attr;
                    $.metadata.setType("attr", attr);
                    targetForm.find('[' + attr + ']').each(function () {
                        if ($(this).metadata().required) {
                            $(this).attr({'aria-required':'true', 'required':'required'});
                        }
                    });
                    targetForm.validate(validateOptions);
                });
            }
            else {
                targetForm.find('[required]').attr({'aria-required':'true'});
                targetForm.validate(validateOptions);
            }
        });
    }
...
adobe.ui.tabs = function (target, defaultTab) {
    var targetID = $("#" + target), dispatcher = adobe.vrbl("reflowDispatcher"), vertTabs = targetID.find('.ui-tabpanel-vertical'), tabsWidth = targetID.width();

    function createTabs() {
        targetID.find('.TabsMobile').hide();
        targetID.tabs({selected:defaultTab});
        if (vertTabs.length == 0) {
            var tabsLI = targetID.find('li.ui-state-default'), tabsCount = tabsLI.length, tabsMaxWidth = tabsWidth / tabsCount, allTabs = 0, pxPerChar;
            if ($.browser.msie || $.browser.opera || typeof CQClientLibraryManager != "undefined") {
                pxPerChar = 7;
            }
            else {
                pxPerChar = 6;
            ...
            tabsLI.each(function () {
                allTabs = $(this).width() + allTabs;
            });
            if (allTabs > tabsWidth) {
                tabsLI.each(function () {
                    $(this).css('maxWidth', tabsMaxWidth);
                    var str = $(this).find('a').text(), strCount = str.length, tabsWholeMaxWidth = Math.floor((tabsMaxWidth - 22)) / pxPerChar;
                    if (strCount > tabsWholeMaxWidth) {
                        str = str.slice(0, tabsWholeMaxWidth - 2);
            ...
            var vertTabsGrid = targetID.find('.ui-tabpanel-vertical .LayoutGrid-4');
            if (vertTabsGrid.length > 0) {
                vertTabsGrid.css('width', vertTabsGrid.innerWidth() + 11);
            }
            vertTabsGrid = targetID.find('.ui-tabpanel-vertical .LayoutGrid-2');
            if (targetID.find('.ui-tabpanel-vertical .LayoutGrid-2').length > 0) {
                vertTabsGrid.css('width', vertTabsGrid.innerWidth() + 11);
            }
            vertTabs.css({minHeight:$('.ui-tabs-vertical').innerHeight()});
            vertTabs.parents('.LayoutFlushLeft').css('width', vertTabs.innerWidth() + 11);
        }
    }

    function reflowTabs() {
        targetID.tabs("destroy");
        var tabLink = '', tabText = '';

        function mobileTabify(me) {
            if (targetID.prev('a[name="' + target + '"]').length == 0) {
                targetID.prepend('<a name="' + target + '"></a>');
            }
            tabLink = $(me).attr('href');
            tabText = ($(me).attr('title') != '') ? $(me).attr('title') : $(me).text();
            if ($(tabLink).length > 0) {
        ...
        if (vertTabs.length == 0) {
            ...
            targetID.find('.ui-tabpanel-vertical .LayoutGrid-4').css('width', '');
            vertTabs.parents('.LayoutFlushLeft').css('width', '');
            targetID.find('ul.ui-tabs-vertical li a').each(function () {
                mobileTabify($(this));
            });
            targetID.find('.TabsMobile').addClass('LayoutCellSides');
        }
    }
...
adobe.reflow = {dispatchers:{}, createDispatcher:function (name, documentElement) {
    return(adobe.reflow.dispatchers[name] = new adobe.reflow.Dispatcher(documentElement || document));
}, getDispatcherByName:function (name) {
    return adobe.reflow.dispatchers[name];
}};
$(document).ready(function () {
    var dispatcher = adobe.vrbl("reflowDispatcher");
    if (dispatcher) {
...
adobe.reflow.Dispatcher = function (domElement) {
    this.layoutEvents = {};
    this.domElement = domElement;
};
adobe.reflow.Dispatcher.prototype = {addLayoutEvent:function (layoutEvent) {
    if (adobe.reflow.LayoutEvent.is(layoutEvent)) {
        this.layoutEvents[layoutEvent.name] = layoutEvent;
    }
    return this;
}, getLayoutEvent:function (name) {
    if (this.hasLayoutEvent(name)) {
        return this.layoutEvents[name];
    }
    return new adobe.reflow.LayoutEvent(name);
}, hasLayoutEvent:function (name) {
    return adobe.reflow.LayoutEvent.is(this.layoutEvents[name]);
}, removeLayoutEvent:function (name) {
    if (this.hasLayoutEvent(name)) {
        delete this.layoutEvents[name];
    }
    return this;
}, updateLayout:function (integer) {
    for (prop in this.layoutEvents) {
        var layoutEvent = this.layoutEvents[prop], active = layoutEvent.isActive(), inRange = layoutEvent.isWithinRange(integer);
        if (active != inRange) {
            layoutEvent.setActive(inRange);
            this.fireLayoutChange(layoutEvent);
        }
    }
    return this;
}, fireLayoutChange:function (layoutEvent) {
    $(this.domElement).trigger("adobe.reflow.LayoutChange", layoutEvent);
}};
adobe.reflow.LayoutEvent = function (name, minRange, maxRange) {
    this.setName(name);
    this.setMinRange(minRange);
    this.setMaxRange(maxRange);
    this.setActive(false);
};
adobe.reflow.LayoutEvent.prototype = {setName:function (name) {
    this.name = String(name);
}, setMinRange:function (integer) {
    this.minRange = parseInt(integer);
}, setMaxRange:function (integer) {
    ...
    this.active = Boolean(yes);
}, isWithinRange:function (integer) {
    var i = parseInt(integer);
    return i >= this.minRange && i <= this.maxRange;
}};
adobe.reflow.LayoutEvent.is = function (func) {
    return func !== undefined && adobe.reflow.LayoutEvent.prototype.isPrototypeOf(func);
};
var swfobject = function () {
    var UNDEF = "undefined", OBJECT = "object", SHOCKWAVE_FLASH = "Shockwave Flash", SHOCKWAVE_FLASH_AX = "ShockwaveFlash.ShockwaveFlash", FLASH_MIME_TYPE = "application/x-shockwave-flash", EXPRESS_INSTALL_ID = "SWFObjectExprInst", ON_READY_STATE_CHANGE = "onreadystatechange", win = window, doc = document, nav = navigator, plugin = false, domLoadFnArr = [main], regObjArr = [], objIdArr = [], listenersArr = [], storedAltContent, storedAltContentId, storedCallbackFn, storedCallbackObj, isDomLoaded = false, isExpressInstallActive = false, dynamicStylesheet, dynamicStylesheetMedia, autoHideShow = true, ua = function () {
        var w3cdom = typeof doc.getElementById != UNDEF && typeof doc.getElementsByTagName != UNDEF && typeof doc.createElement != UNDEF, u = nav.userAgent.toLowerCase(), p = nav.platform.toLowerCase(), windows = p ? /win/.test(p) : /win/.test(u), mac = p ? /mac/.test(p) : /mac/.test(u), webkit = /webkit/.test(u) ? parseFloat(u.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : false, ie = eval("/*@cc_on!@*/false"), playerVersion = [0, 0, 0], d = null;
...
swfobject.replaceSWF = function (swfUrlStr, replaceElemIdStr, widthStr, heightStr, swfVersionStr, xiSwfUrlStr, flashvarsObj, parObj, attObj, callbackFn) {
    var el = document.getElementById(replaceElemIdStr);
    if (el.nodeName == "OBJECT" || el.nodeName == "EMBED") {
        var parent = el.parentNode;
        swfobject.removeSWF(replaceElemIdStr);
        parent.appendChild(document.createElement("div")).setAttribute("id", replaceElemIdStr);
    }
...
$(document).ready(function () {
    ...
    var STORE_URL = 2;
    var PRODUCT_NAME = 3;
    var CART_BUTTON_ID = 4;
    var PRICE_TYPE = 5;
    var TAX_INCLUDE = 6;
    $.each($('.CPodPricePlaceHolder'), function (i, placeHolder) {
        if (placeHolder.attributes.rel != null) {
            var relValue = placeHolder.attributes.rel.value;
            var relArray = relValue.split('|');
            if (relArray.length == 7) {
                displayPrices(relArray[LOCALE], relArray[CATEGORY_PATH], placeHolder, null, relArray[STORE_URL], relArray[CART_BUTTON_ID], relArray[PRODUCT_NAME], relArray[PRICE_TYPE], relArray[TAX_INCLUDE]);
            }
        }
    });
    $('#EDUModalContinuShopping').live("click", function (event) {
        event.preventDefault();
        var button = event.target;
        var eduCB = $(button).parent().parent().find('#eduQualifiedCB');
        var eduCBValue = $(eduCB).val();
        var curMarketSeg = Adobe.Cart.Models.Cart.getMarketSegment();
        var newMarketSeg = null;
        if ($(eduCB).attr('checked') == 'checked') {
            Adobe.Cart.Models.Cart.setMarketSegment("EDU");
            $('#CPodEDUToggle').show();
            $('#CPodEDUSelector').hide();
        } else {
            $('#eduErrorText').show();
            return;
        }
        newMarketSeg = Adobe.Cart.Models.Cart.getMarketSegment();
        if (curMarketSeg != newMarketSeg) {
            OpenAjax.hub.publish("marketsegment.change");
            if (Adobe.PageInfo.ReloadPageOnContinue == null || !Adobe.PageInfo.ReloadPageOnContinue) {
                var url = window.location.href;
                url = Adobe.Product.Controller.CPod.EduController.removeMarketSegmentParam(url);
                window.location = url;
            } else {
                $('#EDUModal-ui').dialog('close');
            }
        } else {
            $('#EDUModal-ui').dialog('close');
        }
...
var conversionpod = (function ($) {
    ...
    this.getMarketSegment = function () {
        var mktSegment = "COM";
        if (Adobe && Adobe.PageInfo && Adobe.PageInfo.marketSegment && Adobe.PageInfo.marketSegment.length) {
            mktSegment = Adobe.PageInfo.marketSegment;
        }
        return mktSegment;
    };
    this.displayPrices = function (countryCode, productPath, vollversion, upgrade, storeUrl, cartButtonId, productName, priceType, taxInclude) {
        var newCC = countryCode.substring(countryCode.indexOf('_') + 1, countryCode.length);
        var marketSegment = getMarketSegment();
        if (marketSegment === "EDU" && productPath.indexOf("STE") === -1) {
            productPath += "STE";
        }
        var key = countryCode + '-' + productPath;
        var cachedItem = $.jCacher.get(key);
        if (cachedItem) {
            printPrices(cachedItem, countryCode, productPath, productName, vollversion, cartButtonId, priceType, taxInclude);
        } else {
            var priceData = {url:storeUrl + '/cfusion/store/services/stateless/jsonshoppingservice.cfc', data:'method=getCategoryPricesByCountry&countryCode=' + newCC + '&categoryPath=' + productPath + '&marketSegment=' + marketSegment + '&uc=1', dataType:'jsonp', jsonp:'callback', jsonpCallback:'getData' + cartButtonId.replace(/-/g, ''), timeout:REQUEST_TIMEOUT, success:function (prices) {
                printPrices(prices, countryCode, productPath, productName, vollversion, cartButtonId, priceType, taxInclude);
    ...
    this.printPrices = function (prices, countryCode, productPath, productName, vollversion, cartButtonId, priceType, taxInclude) {
        var marketSegment = getMarketSegment();
        var isUpgrade = true;
        var displayPriceType = "UPGRADE";
        if (prices != null) {
            var origAndLowestPrice = conversionpod.getOriginalAndLowestPriceItem(prices, countryCode, priceType);
            var price_obj = origAndLowestPrice.lowestItem;
            var originalPrice = origAndLowestPrice.originalPrice;
            if (price_obj != null && price_obj.priceType != undefined) {
                displayPriceType = price_obj.priceType;
            } else if (prices.length == 1) {
                displayPriceType = "FULL";
            }
            if (price_obj != null) {
                ...
                if (queryParams.length > 1) {
                    var indParams = queryParams[1].split("&");
                    for (var i = 0; i < indParams.length; i++) {
                        if (indParams[i].match(/^promoid=/i)) {
                            promoID = indParams[i].substr(8);
                        }
                    }
                }
                if (displayPriceType == "UPGRADE") {
                    $(".CPodUpgrade").show();
                    if (cart.isOverlaySupported()) {
                        $("#" + cartButtonId).bind("click", {distmethod:"UPGRADE", storetype:marketSegment, categorypath:productPath, productname:productName, promoid:promoID}, adobe.fn.handleCartButton);
                    }
                    $(vollversion).replaceWith(conversionpod.getPriceDisplayHTML(price_obj, originalPrice, taxInclude));
                    return false;
                } else if (displayPriceType == "FULL") {
                    $(vollversion).replaceWith(conversionpod.getPriceDisplayHTML(price_obj, originalPrice, taxInclude));
                    if (cart.isOverlaySupported()) {
                        $("#" + cartButtonId).bind("click", {distmethod:"FULL", storetype:marketSegment, categorypath:productPath, productname:productName, promoid:promoID}, adobe.fn.handleCartButton);
                    }
                    return false;
                } else if (displayPriceType == "SUB_NEW") {
                    $(".CPodUpgrade").show();
                    if (cart.isOverlaySupported()) {
                        $("#" + cartButtonId).bind("click", {distmethod:"SUB_NEW", storetype:marketSegment, categorypath:productPath, productname:productName, promoid:promoID}, adobe.fn.handleCartButton);
                    }
                    $(vollversion).replaceWith(conversionpod.getPriceDisplayHTML(price_obj, originalPrice, taxInclude));
                    return false;
                }
    ...
    this.getOriginalAndLowestPriceItem = function (prices, countryCode, priceType) {
        function getRawPrice(pObj, price_string) {
            price_string = price_string.replace(pObj.CURRENCYSYMBOL, "").replace(pObj.TAXEXCLUSIVELABEL, "").replace(pObj.TAXINCLUSIVELABEL, "");
            return trimSpace(price_string);
        }

        function getPriceDollar(rawPrice, dollar_delimitor, cent_delimitor) {
            var tempvar = rawPrice.split(dollar_delimitor);
            if (cent_delimitor != "") {
                var tempvarlast = tempvar[tempvar.length - 1].split(cent_delimitor);
                tempvar[tempvar.length - 1] = tempvarlast[0];
            }
            return tempvar;
        }
        ...
        function getPriceCent(rawPrice, cent_delimitor) {
            var centStr = "";
            if (cent_delimitor != "") {
                var tempvar = rawPrice.split(cent_delimitor);
                centStr = trimSpace(tempvar[1]);
            }
            return centStr;
        }
        ...
        for (var i = 0; i < prices.length; i++) {
            if (prices[i]['PRICETYPE'] != null && prices[i]['PRICETYPE'] != undefined) {
                if (priceType.toLowerCase() == prices[i]['PRICETYPE'].toLowerCase()) {
                    lowestItem = adobe2.ecomm.Format.getFormattedPriceObj(prices[i], countryCode.toLowerCase());
                    origPrice = {dollarAmtTaxInc:prices[i].ORIGINALPRICEWITHTAX != "" ? getPriceDollar(getRawPrice(prices[i], prices[i].ORIGINALPRICEWITHTAX), lowestItem.dollarDelim, lowestItem.centDelim) : "", centAmtTaxInc:prices[i].ORIGINALPRICEWITHTAX != "" ? getPriceCent(getRawPrice(prices[i], prices[i].ORIGINALPRICEWITHTAX), lowestItem.centDelim) : "", dollarAmtTaxExc:prices[i].ORIGINALPRICEWITHOUTTAX != "" ? getPriceDollar(getRawPrice(prices[i], prices[i].ORIGINALPRICEWITHOUTTAX), lowestItem.dollarDelim, lowestItem.centDelim) : "", centAmtTaxExc:prices[i].ORIGINALPRICEWITHOUTTAX != "" ? getPriceCent(getRawPrice(prices[i], prices[i].ORIGINALPRICEWITHOUTTAX), lowestItem.centDelim) : ""};
                    origAndLowestPrice = {originalPrice:origPrice, lowestItem:lowestItem};
                    break;
                } else {
                    var price_obj = adobe2.ecomm.Format.getFormattedPriceObj(prices[i], countryCode.toLowerCase());
                    if (lowestPrice == -1 || lowestPrice > Number(price_obj.dollarAmtTaxExc)) {
                        lowestPrice = Number(price_obj.dollarAmtTaxExc);
                        lowestItem = price_obj;
                    }
                }
            } else {
                var price_obj = adobe2.ecomm.Format.getFormattedPriceObj(prices[i], countryCode.toLowerCase());
                if ((price_obj != null) && (lowestPrice == -1 || lowestPrice > Number(price_obj.dollarAmtTaxExc))) {
                    lowestPrice = Number(price_obj.dollarAmtTaxExc);
                    lowestItem = price_obj;
                }
            }
        ...
        return origAndLowestPrice;
    };
    this.getPriceDisplayHTML = function (priceObj, originalPrice, taxInclude) {
        function getDollarPriceHTML(dollarPrice, localeDelimiter) {
            var tempValue = "";
            for (i = 0; i < dollarPrice.length; i++) {
                tempValue = tempValue + dollarPrice[i] + localeDelimiter;
            }
            return tempValue.substring(0, tempValue.length - 1);
        }

        var symbol = priceObj.symbol;
        var dPrice = priceObj.dollarAmtTaxExc;
        var cPrice = priceObj.centAmtTaxExc;
        var taxLabel = priceObj.taxLabelExc;
        var dOrigPrice = originalPrice.dollarAmtTaxExc;
        var cOrigPrice = originalPrice.centAmtTaxExc;
        if (taxInclude.length > 0 && taxInclude == "true") {
            dPrice = priceObj.dollarAmtTaxInc;
            cPrice = priceObj.centAmtTaxInc;
            taxLabel = priceObj.taxLabelInc;
            dOrigPrice = originalPrice.dollarAmtTaxInc;
            cOrigPrice = originalPrice.centAmtTaxInc;
        }
        var priceClass = PRICE_MAIN_CLASS;
        if (PRICE_MAX_THRESHOLD < (symbol.lenght + dPrice.length + cPrice.length)) {
            priceClass = PRICE_SMALL_CLASS;
        }
        if (dOrigPrice != null && dOrigPrice != 'undefined' && dOrigPrice != "") {
            var origPriceObj = {"text":"<div><span class=\"TextStrikeThrough\">" + "<span class=\"CPodCurrency \">" + symbol + "</span>" + "<span class=\"" + priceClass + " \">" + getDollarPriceHTML(dOrigPrice, priceObj.dollarDelim) + "</span>" + "<span>" + cOrigPrice + "</span></span></div>"}
        }
        var tempObj = {"sp":"", "TXLAB":"", "SYM":"<span class=\"CPodCurrency\">" + symbol + "</span>", "DDDD":"<span class=\"" + priceClass + "\">" + getDollarPriceHTML(dPrice, priceObj.dollarDelim) + "</span>", "CC":cPrice};
        if (origPriceObj != null && origPriceObj != 'undefined') {
            return origPriceObj.text + (priceObj.mask.replace(new RegExp("(sp|SYM|TXLAB|DDDD|CC)", "gi"), function ($1) {
                return(tempObj[$1]);
            }))
        }
        else {
            return priceObj.mask.replace(new RegExp("(sp|SYM|TXLAB|DDDD|CC)", "gi"), function ($1) {
                return(tempObj[$1]);
            })
        }
    };
...
function openPopUpWindow(urlstr, w, h, optionsstr, namestr) {
    ...
    options = (optionstest == "") ? "toolbar=no,location=no,menubar=no,scrollbars=yes,resizable=yes" : optionsstr;
    name = (typeof namestr == "undefined") ? "_blank" : namestr;
...
function mmDownload(downloadURL) {
    if (adobe.hostEnv["ieV"] != 7) {
        try {
            frames['download'].location.replace(downloadURL);
        } catch (e) {
            document.getElementById('download').src = downloadURL;
        }
    }
    else {
        downloadWindow = window.open(downloadURL, "_blank", "directories=0,location=0,menubar=0,resizable=0,scrollbars=0,status=1,toolbar=0,width=800,height=600");
    }
}
...
(function (jQ) {
    jQ.extend({createNs:function (ns) {
        var o, a;
        a = ns.split(".");
        o = window[a[0]] = window[a[0]] || {};
        jQ.each(a.slice(1), function (i, n) {
            o = o[n] = o[n] || {};
        });
        return o;
        ...
        if (mode) {
            switch (mode) {
                case 1:
                    return jQ.mix(r.prototype, s.prototype, ov, wl, 0, merge);
                case 2:
                    jQ.mix(r.prototype, s.prototype, ov, wl, 0, merge);
                    break;
                case 3:
                    return jQ.mix(r, s.prototype, ov, wl, 0, merge);
                case 4:
                    return jQ.mix(r.prototype, s, ov, wl, 0, merge);
                default:
            }
        }
        var arr = merge && jQ.isArray(r), i, l, p;
        if (wl && wl.length) {
            for (i = 0, l = wl.length; i < l; ++i) {
                p = wl[i];
                if (p in s) {
                    if (merge && jQ.isPlainObject(r[p], true)) {
                        jQ.mix(r[p], s[p]);
            ...
            for (i in s) {
                if (merge && jQ.isPlainObject(r[i], true)) {
                    jQ.mix(r[i], s[i]);
            ...
            if (jQ.hasKey(jQ.browser, "msie")) {
                var fn = s.toString;
                if (jQ.isFunction(fn) && fn != Object.prototype.toString) {
                    r.toString = fn;
                }
            }
        }
...
(function (jQ) {
    jQ.extend({"Array":{hash:function (k, v) {
        var o = {}, l = k.length, vl = v && v.length, i;
        for (i = 0; i < l; i = i + 1) {
            o[k[i]] = (vl && vl > i) ? v[i] : true;
        }
        return o;
    }, filter:(Array.prototype.filter) ? function (a, f, o) {
        return Array.prototype.filter.call(a, f, o);
    } : function (a, f, o) {
        var results = [];
        jQ.Array.each(a, function (item, i, a) {
            if (f.call(o, item, i, a)) {
                results.push(item);
            }
        });
        return results;
    }, each:(Array.prototype.forEach) ? function (a, f, o) {
        Array.prototype.forEach.call(a || [], f, o || jQ);
        return jQ;
    } : function (a, f, o) {
        ...
        return jQ;
    }, some:(Array.prototype.some) ? function (a, f, o) {
...
(function (jQ) {
    jQ.extend({"UI":{setControlEnabled:function (control, enable) {
        if (enable)
            control.removeAttr("disabled"); else
            control.attr("disabled", "disabled");
    }}});
})(jQuery);
jQuery.createNs("adobe2.ecomm");
adobe2.ecomm.Format = {getFormattedPriceObj:function (pObj, locale) {
    ...
    switch (locale) {
        ...
        case'en_no':
            ...
            break;
        case'en_nz':
            tempObj = {browsePriceDisplay:"TAX_EXCLUSIVE", dollarAmtTaxInc:pObj.PRICEWITHTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHTAX), ',', '.') : [], centAmtTaxInc:pObj.PRICEWITHTAX != "" ? getPriceCent(getRawPrice(pObj.PRICEWITHTAX), '.') : "", dollarAmtTaxExc:pObj.PRICEWITHOUTTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHOUTTAX), ',', '.') : [], centAmtTaxExc:pObj.PRICEWITHOUTTAX != "" ? getPriceCent(getRawPrice(pObj.PRICEWITHOUTTAX), '.') : "", dollarDelim:",", centDelim:".", symbol:pObj.CURRENCYSYMBOL != "" ? trimSpace(pObj.CURRENCYSYMBOL) : "", taxLabelExc:pObj.TAXEXCLUSIVELABEL != "" ? pObj.TAXEXCLUSIVELABEL : "", taxLabelInc:pObj.TAXINCLUSIVELABEL != "" ? pObj.TAXINCLUSIVELABEL : "", mask:"SYMDDDDCC", priceType:pObj.PRICETYPE};
            return tempObj;
            break;
        case'en_pt':
            tempObj = {browsePriceDisplay:"BOTH", dollarAmtTaxInc:pObj.PRICEWITHTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHTAX), ',', '.') : [], centAmtTaxInc:pObj.PRICEWITHTAX != "" ? getPriceCent(getRawPrice(pObj.PRICEWITHTAX), '.') : "", dollarAmtTaxExc:pObj.PRICEWITHOUTTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHOUTTAX), ',', '.') : [], centAmtTaxExc:pObj.PRICEWITHOUTTAX != "" ? getPriceCent(getRawPrice(pObj.PRICEWITHOUTTAX), '.') : "", dollarDelim:",", centDelim:".", symbol:pObj.CURRENCYSYMBOL != "" ? trimSpace(pObj.CURRENCYSYMBOL) : "", taxLabelExc:pObj.TAXEXCLUSIVELABEL != "" ? pObj.TAXEXCLUSIVELABEL : "", taxLabelInc:pObj.TAXINCLUSIVELABEL != "" ? pObj.TAXINCLUSIVELABEL : "", mask:"SYMspDDDDCCspTXLAB", priceType:pObj.PRICETYPE};
        ...
        case'it_it':
            ...
            break;
        case'ja_jp':
            tempObj = {browsePriceDisplay:"BOTH", dollarAmtTaxInc:pObj.PRICEWITHTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHTAX), ',', '') : [], centAmtTaxInc:"", dollarAmtTaxExc:pObj.PRICEWITHOUTTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHOUTTAX), ',', '') : [], centAmtTaxExc:"", dollarDelim:",", centDelim:"", symbol:pObj.CURRENCYSYMBOL != "" ? trimSpace(pObj.CURRENCYSYMBOL) : "", taxLabelExc:pObj.TAXEXCLUSIVELABEL != "" ? pObj.TAXEXCLUSIVELABEL : "", taxLabelInc:pObj.TAXINCLUSIVELABEL != "" ? pObj.TAXINCLUSIVELABEL : "", mask:"DDDDspSYMspTXLAB", priceType:pObj.PRICETYPE};
            return tempObj;
            break;
        case'pt_br':
            tempObj = {browsePriceDisplay:"TAX_EXCLUSIVE", dollarAmtTaxInc:pObj.PRICEWITHTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHTAX), '.', ',') : [], centAmtTaxInc:pObj.PRICEWITHTAX != "" ? getPriceCent(getRawPrice(pObj.PRICEWITHTAX), ',') : "", dollarAmtTaxExc:pObj.PRICEWITHOUTTAX != "" ? getPriceDollar(getRawPrice(pObj.PRICEWITHOUTTAX), '.', ',') : [], centAmtTaxExc:pObj.PRICEWITHOUTTAX != "" ? getPriceCent(getRawPrice(pObj.PRICEWITHOUTTAX), ',') : "", dollarDelim:".", centDelim:",", symbol:pObj.CURRENCYSYMBOL != "" ? trimSpace(pObj.CURRENCYSYMBOL) : "", taxLabelExc:pObj.TAXEXCLUSIVELABEL != "" ? pObj.TAXEXCLUSIVELABEL : "", taxLabelInc:pObj.TAXINCLUSIVELABEL != "" ? pObj.TAXINCLUSIVELABEL : "", mask:"SYMspDDDDCCspTXLAB", priceType:pObj.PRICETYPE};
...
}, getPriceHTML:function (pObjforHTML, locale) {
}};
function saveRegionCookie(region) {
    cart.setStoreRegion(region);
}
function saveUbiCartCookie() {
    var cookie = new adobe.http.Cookie("UBICART", window.location.href.replace(/index_.\.html/, ""));
    cookie.setExpires(86400002 * 365);
    cookie.setPath("/");
    cookie.setDomain(".adobe.com");
    window.document.cookie = cookie.toString();
}
var s_products = '';
var s_eVar45 = '';
var s_events = '';
...
;
$(document).ready(function () {
    var OmUrl = window.location.toString();
    OmUrl = unescape(OmUrl.replace(/_sl_/g, "/"));
    $('.QuickJump a').click(function () {
        OmObject = new Object();
        OmObject.pageName = OmUrl.toLowerCase() + " JUMP: " + $(this).attr('href');
        s.t(OmObject);
    });
    $('.jcarousel-next').click(function () {
        OpenAjax.hub.publish("carousel.next.click");
    });
    $('.jcarousel-prev').click(function () {
        OpenAjax.hub.publish("carousel.previous.click");
    });
    function OmSocialMedia(OmSMType) {
        var s = s_gi(s_account), OmArea = $('meta[name=area]').attr("content");
        s.linkTrackVars = "prop36";
        switch (OmArea) {
            case'devnet':
                s.prop36 = s_prop36 = "ADC: Share " + OmSMType + ": " + window.location.pathname;
                s.tl(this, "o", "ADC: Share " + OmSMType + ": " + window.location.pathname);
                break;
            case'products':
                s.prop36 = "Share: " + OmSMType + ": " + window.location.pathname;
                s.tl(this, "o", "Social Plugin Share: " + OmSMType);
                break;
            default:
                s.prop36 = "Share: " + OmSMType + ": " + window.location.pathname;
                s.tl(this, "o", "Social Plugin Share: " + OmSMType);
    ...
    $('.Facebook-button').click(function () {
        OmSocialMedia("Facebook");
    });
    $('.Twitter-button').click(function () {
        OmSocialMedia("Twitter");
    });
...
var SearchBuddy = (function () {
    _SearchBuddyInstance = function () {
        _SearchBuddy = {};
        _SearchBuddy.IDS = {gnavForm:"#globalnav-search", siteSearch:"#site-search", searchInput:"#search-input", searchBuddySubmit:"search-buddy-submit", searchLocField:"#searchbuddy-loc", resultsContainer:"sb-results", resultsBody:"sb-results-body", resultsList:"sb-result-list"};
        _SearchBuddy.CSS = {activated:"activated", highlight:"highlight", noResult:"no-result", searchResult:"search-result", pulloutItem:"pullout-item", pulloutContent:"pullout-content", linkList:"link-list", searchSuggestion:"search-suggestion", searchAll:"txtright search-all"};
        this.initialize();
    };
    _SearchBuddyInstance.prototype = {initialize:function (URLS, STRINGS) {
        if ($(_SearchBuddy.IDS.gnavForm) == null)return;
        if (!$(_SearchBuddy.IDS.gnavForm).hasClass("searchbuddy"))return;
        this.URLS = $.extend({searchBuddy:"/cfusion/search/buddy/searchbuddy.cfm", searchResults:"/cfusion/search/index.cfm"}, URLS);
        ...
        if ($(_SearchBuddy.IDS.searchLocField).val() == null) {
            this.loc = "en_us";
        } else {
            this.loc = $(_SearchBuddy.IDS.searchLocField).val();
        }
        this.locJSON = this.loc;
        this.input.attr("autocomplete", "off");
        this.term = this.input.val();
        if (this.loc == "en_us") {
            this.STRINGS = $.extend({searchForElipsis:"Search for...", searchFor:"Search for", seeAllResults:"See all search results &#8250;", delay:0.015}, STRINGS);
        }
        else if (this.loc == "de") {
            this.locJSON = "de_de";
            this.STRINGS = $.extend({searchForElipsis:"Suchen...", searchFor:"Suchen", seeAllResults:"Alle Suchergebnisse anzeigen &#8250;", delay:0.015}, STRINGS);
        }
        else if (this.loc == "es_es") {
            this.STRINGS = $.extend({searchForElipsis:"Buscar...", searchFor:"Buscar", seeAllResults:"Ver todos los resultados de búsqueda &#8250;", delay:0.015}, STRINGS);
        }
        else if (this.loc == "fr_fr") {
            this.STRINGS = $.extend({searchForElipsis:"Rechercher...", searchFor:"Rechercher", seeAllResults:"Voir les résultats de la recherche &#8250;", delay:0.015}, STRINGS);
        }
        else if (this.loc == "ja_jp") {
            this.STRINGS = $.extend({searchForElipsis:"検索...", searchFor:"検索", seeAllResults:"すべての検索結果を見る &#8250;", delay:0.015}, STRINGS);
        }
        ...
        var siteSection_input = $("<input>", {"name":"siteSection", "type":"hidden", "value":this.siteSection});
        siteSection_input.appendTo(this.gnavForm);
    }, handleFormSubmit:function (event) {
        var keyCode = event.keyCode || event.which;
        this.gnavForm.attr('action', '/go/gnav_search');
        ...
        this.keyMap = {SHIFT:e.shiftKey, ALT:e.altKey, CTRL:e.ctrlKey, ESC:keyEvent == 27, RETURN:keyEvent == 13, TAB:keyEvent == 9, DELETE:keyEvent == 8 || keyEvent == 46, DOWN:keyEvent == 40, UP:keyEvent == 38, ALPHANUMERIC:((keyEvent >= 48 && keyEvent <= 57) || (keyEvent >= 65 && keyEvent <= 90))};
        ...
        if (this.keyMap.UP) {
            ...
            if (this.selectedIndex > 1 && this.selectedIndex < resultLinks.length) {
                ...
                this.goURL = linkString;
            }
        }
    }, setTerm:function () {
        this.term = this.input.val();
        this.term = this.term.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        if (this.term == "") {
            this.deactivateMenu();
        } else {
            _search = this.search(this.term);
            setTimeout('_search', this.STRINGS.delay);
        }
    }, search:function () {
        ...
        var dd = $('<dd/>', {'class':_SearchBuddy.CSS.noResult}).html($('<a/>', {'href':ddLink}).html(ddText));
        ...
        $.each(hublets, function (index, hub) {
            ...
            var image = $('<div/>', {'class':_SearchBuddy.CSS.pulloutItem}).html($('<a/>', {'href':hub.HOMEPAGEURL}).html($('<img>', {'src':imgURL})));
            var content = $('<div/>', {'class':_SearchBuddy.CSS.pulloutContent});
            var h4 = $('<h4/>', {'href':hub.HOMEPAGEURL}).html($('<a/>', {'href':hub.HOMEPAGEURL}).html(hub.TITLE));
        ...
        var searchAllLink = $('<a/>', {'id':_SearchBuddy.IDS.searchBuddySubmit}).html(this.STRINGS.seeAllResults).bind({'click':$.proxy(this, "handleFormSubmit"), 'keydown':$.proxy(this, "handleFormSubmit")});
